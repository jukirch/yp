...
sap.ui.define([
    ...
    function createDeleteOperation(sNameSpace, sPath, oObjectToUpdate, sPropertyName) {
        ...
        if (oObjectToUpdate) {
             ...
             */
            if (sNameSpace === CommonUtils._getLayerNamespace() + ".settings") {
                oPayLoadUtils.aEntityPropertyChange.push(
                    createEntityPropertyChangeObject(sPath, "UPSERT", {
                        operation: "DELETE"
                    })
                );
            } else {
    ...
    function createCustomerSettingsChange(oCardManifest) {
         ...
         */
        oPayLoadUtils.aEntityPropertyChange.push(
            createEntityPropertyChangeObject(
                CommonUtils._getLayerNamespace() + ".settings",
                "UPSERT",
     ...
     */
    function createTextPropertyChange(
        sIdPath,
        sPath,
        sPropertyName,
        sValue,
        ...
        if (oObjectToUpdate) {
            ...
            oPayLoadUtils.aEntityPropertyChange.push(
                createEntityPropertyChangeObject(sPropPath, "UPSERT", "{{" + sId + "}}")
            );
        }
    }
    ...
    function createTextChangesForSimpleCard(sNameSpace, oCardManifest, aTexts, bObjectToUpdate) {
        ...
        for (var i = 0; i < aTexts.length; i++) {
             ...
             */
            if (
                (!oldSettings[aTexts[i]] && newSettings[aTexts[i]]) ||
                (oldSettings[aTexts[i]] &&
                    newSettings[aTexts[i]] &&
                    oldSettings[aTexts[i]] != newSettings[aTexts[i]])
            ) {
                /**
                 *  If the property has to be updated on the object
                 */
                if (bObjectToUpdate) {
                    createTextPropertyChange(
                        sNameSpace,
                        sNameSpace,
                        aTexts[i],
                        newSettings[aTexts[i]],
                        newSettings,
                        oCardManifest[sNameSpace]
                    );
                } else {
                    /**
                     *  Else create an entity property change payLoad
                ...
                if (!layer || layer === OVPUtils.Layers.customer) {
                    ...
                    if (oI18nValueProperty[aTexts[i]]) {
                        oCardManifest.settings[aTexts[i]] =
                            this._oCardManifestSettings[oI18nValueProperty[aTexts[i]]];
                    } else {
                        oCardManifest.settings[aTexts[i]] = newSettings[aTexts[i]];
                    }
                }
    ...
    function createPropChangesForSimpleCard(sNameSpace, oCardManifest, aSettings, bObjectToUpdate) {
        ...
        for (var i = 0; i < aSettings.length; i++) {
             ...
             */
            if (
                (!oldSettings[aSettings[i]] && newSettings[aSettings[i]]) ||
                (oldSettings[aSettings[i]] &&
                    newSettings[aSettings[i]] &&
                    oldSettings[aSettings[i]] != newSettings[aSettings[i]]) || 
                    (typeof oldSettings[aSettings[i]] === "boolean" && typeof newSettings[aSettings[i]] === "boolean" 
                    && oldSettings[aSettings[i]] != newSettings[aSettings[i]])
            ) {
                /**
                 *  If the property has to be updated on the object
                ...
                if (newSettings[i][aTexts[j]]) {
                    ...
                    if (bObjectToUpdate) {
                        createTextPropertyChange(
                            ...
                            sPath,
                            aTexts[j],
                            newSettings[i][aTexts[j]],
                            newCardSettings,
                            oCardManifest[sNameSpace][sType][i]
                        );
                    } else {
                        /**
     ...
     *
     */
    function createChangeForRevealOfArray(
        sNameSpace,
        oCardManifest,
        newArray,
        aTexts,
         ...
         *  Reveal whole Array
         */
        createTextChangeForWholeArray.bind(this)(
            sNameSpace,
            oCardManifest,
            newArray,
            aTexts,
    ...
    function createChangeForArray(sNameSpace, oCardManifest, newArray, oldArray, sType, bObjectToUpdate) {
        ...
        if (newArray && oldArray) {
             ...
             */
            createChangeForRemovalOfArray.bind(this)(
                sNameSpace,
                oCardManifest,
                aTexts,
                aSettings,
            ...
            createChangeForRevealOfArray.bind(this)(
                sNameSpace,
                oCardManifest,
                aCopyNewArray,
                aTexts,
    ...
    function createChangesForComplexCard(sNameSpace, oCardManifest, bObjectToUpdate) {
        ...
        if (aStaticContent || aOriginalStaticContent) {
            createChangeForArray.bind(this)(
                sNameSpace,
                oCardManifest,
                aStaticContent,
                aOriginalStaticContent,
                "staticContent",
                bObjectToUpdate
            );
    ...
    function createManifestForKPICard(settingsUtils) {
        var aAllCardSettings = SettingsConstants.AllCardSettingsForKPICard,
            j,
            ...
            oCardManifest = {
                model:
                    ...
                    ".kpi_card_model_" +
                    settingsUtils.getTrimmedDataURIName(newSettings.selectedKPI.ODataURI),
    ...
    function createManifestForStaticLinkListCard() {
        var aStaticContent = this._oCardManifestSettings["staticContent"],
            aTexts = SettingsConstants.cardSettingsArrayLevel["staticContent"]["text"],
            i,
            j,
    ...
    function createManifestForNewCard(settingsUtils) {
        var aAllCardSettings = SettingsConstants.NewCardSettings,
            aSelectedCardSetting;
        var newSettings = this._oCardManifestSettings;
        aAllCardSettings.forEach(
            function (settings) {
                ...
                if (!settingsUtils.bAddNewCardFlag) {
                    ...
                    if (newSettings[aCardSettings[j]]) {
                        if (oI18nValueProperty[aCardSettings[j]]) {
                            oCardManifest.settings[aCardSettings[j]] =
                                this._oCardManifestSettings[oI18nValueProperty[aCardSettings[j]]];
                        } else {
                            oCardManifest.settings[aCardSettings[j]] = newSettings[aCardSettings[j]];
                        }
                    }
    ...
    function createTextTranslationObjectForCloneCard(sApplicationId, sCardId, oCardManifest, oSettingsUtils) {
        var oText,
            ...
            k,
            sPropertyName,
            aStaticContent = oCardManifest.settings["staticContent"],
                        ...
                        if (!oSettingsUtils.bAddNewCardFlag) {
                            ...
                            for (var key in oI18nKeyValueProperty) {
                                if (aCardSettingsWithText[i] === key) {
                                    sId = this._oCardManifestSettings[oI18nKeyValueProperty[key]]
                                        ? this._oCardManifestSettings[oI18nKeyValueProperty[key]]
                                        : generateId(sApplicationId, sCardId, "settings", aCardSettingsWithText[i]);
                                    break;
                                }
                            }
                        }
    ...
    function getFinalPayload(settingsUtils, cardType) {
        var oParameters = {
            ...
            oCardManifest,
            oFinalData = {},
            bFioriToolsRTAMode = CommonUtils.getQueryParameterValue("fiori-tools-rta-mode");
        ...
        var cardId = CommonUtils._getLayerNamespace() + cardType,
            i = 1;
        ...
        oFinalData.oText = createTextTranslationObjectForCloneCard.bind(this)(
            sApplicationId,
            cardId,
            oCardManifest,
            settingsUtils
    ...
    var oPayLoadUtils = {
        ...
        getPayLoadForEditCard: function (settingsUtils) {
            var sCardId = settingsUtils.oAppDescriptor.id,
                oParameters = {
                    cardId: sCardId,
                },
                oText,
                aEntityPropertyChange,
                oCardManifest = merge({}, settingsUtils.oAppDescriptor),
            ...
            sNameSpace =
                sCardId.lastIndexOf(CommonUtils._getLayerNamespace() + ".", 0) === 0
                    ? "settings"
                    : CommonUtils._getLayerNamespace() + ".settings";
        ...
        getPayLoadForCloneCard: function (oComponentContainer) {
            return new Promise(function (resolve, reject) {
                ...
                var cardId = CommonUtils._getLayerNamespace() + "." + oCardManifest.id,
                    i = 1;
        ...
        getPayLoadForNewCard: function (settingsUtils) {
            //TODO: make a method to return the new data source payload for new card and new kpi card
        ...
        getPayLoadForRemoveCard: function (oComponentContainer) {
            ...
            if (
                !checkIfOtherCardUsesTheSameModel(oComponentData.modelName, sCardId, aCards) &&
                !!oComponentData.modelName
            ) {
                ...
                var oMetaData = oComponentData.appComponent.getMetadata(),
                    ...
                    aAnnotationNames =
                        oMetaData.getManifestEntry("sap.app").dataSources[sDataSource].settings.annotations;
