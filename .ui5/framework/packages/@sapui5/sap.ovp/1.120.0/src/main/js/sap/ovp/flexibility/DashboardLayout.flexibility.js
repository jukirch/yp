            ...
            changeHandler: {
                    ...
                    if (oCard) {
                        ...
                        if (typeof iNewRowValue === "number") {
                            if (iNewRowValue === 0) {
                                ...
                                if (
                                    oCardProperties.template === "sap.ovp.cards.list" ||
                                    oCardProperties.template === "sap.ovp.cards.table"
                                ) {
                ...
                revertChange: function (oChange, oControl, mPropertyBag) {
                    var oMainController = CommonUtils.getApp(),
                        ...
                        bShowOnlyHeader = oCardProperties.settings.defaultSpan &&
                                            oCardProperties.settings.defaultSpan.showOnlyHeader,
                        affectedCards = [];
        ...
        newCardSettings: {
            changeHandler: {
                applyChange: function (oChange, oControl, mPropertyBag) {
                    ...
                    var oNewComponentContainer = new ComponentContainer(
                        oMainView.getId() + "--" + oCardProperties.id
                    ),
                        ...
                        oLayoutUtil = oMainLayout.getDashboardLayoutUtil(),
                        oLayoutModel = oMainLayout.getDashboardLayoutModel(),
                        bNewStaticLinkListCard =
                            oCardProperties.id.indexOf("newStaticLinkListCard_N") !== -1 &&
                            !SettingsUtils.checkClonedCard(oCardProperties.id),
                        bNewKPICard =
                            oCardProperties.id.indexOf("newKPICard_N") !== -1 &&
                            !SettingsUtils.checkClonedCard(oCardProperties.id),
                        bAddNewCard =
                            oCardProperties.id.indexOf("newCard_N") !== -1 &&
                            !SettingsUtils.checkClonedCard(oCardProperties.id),
                        bAddNewCardFromNewDataSource = SettingsUtils.newDataSource;
                    if (bNewKPICard) {
                        var oSelectedKPI = oCardProperties.settings.selectedKPI,
                            oNewModel = new ODataModel(oSelectedKPI.ODataURI, {
                        ...
                        if (oCardProperties.settings["sAnnoKey"]) {
                            SettingsUtils.setDataSources(
                                oCardProperties.settings["sAnnoKey"],
                                oSelectedKPI.ModelURI
                            );
                        }
                    ...
                    if (bAddNewCardFromNewDataSource && !oAppComponent.getModel(oCardProperties.model)) {
                        var oNewCardModel = new ODataModel(
                            SettingsUtils.newDataSourceModel.serviceURI,
                            {
                            ...
                            sModelName = oCardProperties.model;
                        SettingsUtils.setDataSources(
                            SettingsUtils.newDataSourceModel.serviceAnnotation,
                            SettingsUtils.newDataSourceModel.serviceAnnotationURI
                        );
                    ...
                    for (i = 0; i < aCards.length; i++) {
                        if (
                            oCardProperties.id.lastIndexOf(
                                CommonUtils._getLayerNamespace() + "." + aCards[i].id,
                                0
                            ) === 0
                        ) {
                            iIndex = i;
                            ...
                            var iCloneCardColumn =
                                oCardProperties.dashboardLayout.column + oCardProperties.dashboardLayout.colSpan;
                            //If the card is present at the last column then the clone card is created at 1st column
                            // else besides to the card
                            oCardProperties.dashboardLayout.column =
                                iCloneCardColumn < oLayoutModel.getColCount() ? iCloneCardColumn : 1;
                            break;
                        }
                    }
                    ...
                    setTimeout(function () {
                        ...
                        oOverLay.focus();
                        var sMessage =
                            bNewStaticLinkListCard || bNewKPICard || bAddNewCard
                                ? OvpResources.getText("OVP_KEYUSER_TOAST_MESSAGE_FOR_NEW")
                ...
                revertChange: function (oChange, oControl, mPropertyBag) {
                    ...
                    if (oCard) {
                        var oComponent = oCard.getComponentInstance(),
                            ...
                            bNewKPICard =
                                oComponentData.cardId.indexOf("newKPICard_N") !== -1 &&
                                !SettingsUtils.checkClonedCard(sCardId);
                        if (bNewKPICard) {
