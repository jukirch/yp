...
import type { AnnotationTableColumn, TableFiltersConfiguration, TableVisualization } from "sap/fe/core/converters/controls/Common/Table";
...
import type MicroChartBlock from "../microchart/MicroChart.block";
...
const TableHelper = {
	...
	create$Select: function (table: TableBlock) {
		...
		if (lineItemContext.getPath().includes(`@${UIAnnotationTerms.LineItem}`)) {
	 ...
	 */
	_shouldIncludeHeaderInColumnwidhCalculation(table: tableDelegateModel, column: AnnotationTableColumn): boolean | undefined {
	...
	getColumnWidthForDataField: function (
		...
		dataFieldActionText: string,
		dataModelObjectPath: DataModelObjectPath,
		microChartTitle?: MicroChartBlock
	): number | null {
	 ...
	 * @returns The stable ID for a given column
	 */
	getColumnStableId: function (id: string, dataModelObjectPath: DataModelObjectPath): string | undefined {
