...
export default class DataPointBlock extends BuildingBlockBase {
	...
	private static getTemplatingObjects(context: DataPointProperties | DataPointBlock): {
		...
		const internalDataPointConverted = convertMetaModelContext(context.metaPath) as DataPointType;
	...
	getProgressIndicatorTemplate() {
		...
		if (this?.formatOptions?.showLabels ?? false) {
			...
			const secondLabelExpression = (valueDataModelPath?.targetObject as Property)?.annotations?.Common?.Label?.toString();
	...
	private getTooltipValue() {
		...
		return getExpressionFromAnnotation(dataPointConverted?.annotations?.Common?.QuickInfo, getRelativePaths(dataModelPath));
	}
