...
import type { FilterBarState } from "sap/fe/core/controllerextensions/ViewState";
...
import stateFilterToSelectionVariant from "../filterBar/adapter/StateFilterToSelectionVariant";
 ...
 */
export function isInsightsCardCreationPossible(filterbarId: string, insightsRelevantColumns?: TableCardColumn[]): boolean {
	if (insightsRelevantColumns && insightsRelevantColumns.length === 0) {
		return false;
	}
	const filterBar = Core.byId(filterbarId);
	if (filterBar?.isA("sap.fe.core.controls.FilterBar")) {
...
export async function createInsightsParams(
	...
	const filterBar = Core.byId(filterBarId) as FilterBar | undefined;
	const control = controlAPI.content;
	const controlState = (await StateUtil.retrieveExternalState(control as Table)) as FilterBarState; // define type for control state
	const controlSV = stateFilterToSelectionVariant.getSelectionVariantFromConditions(
		...
		(control as Table).getPropertyHelper()
	);
	...
	const insightsParams: InsightsParams<Partial<TableContent | ChartContent>> = {
		...
		parentAppManifest: appComponent.getManifest(),
		parameters: {
...
export function hasInsightActionEnabled(actions: ActionToolbarAction[]): boolean {
