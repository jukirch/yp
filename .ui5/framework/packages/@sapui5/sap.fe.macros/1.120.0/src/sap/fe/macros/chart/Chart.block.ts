...
const setCustomActionProperties = function (childAction: Element) {
	...
	if (
		...
		action.namespaceURI &&
		["sap.fe.macros", "sap.fe.macros.chart"].includes(action.namespaceURI)
	) {
...
export default class ChartBlock extends BuildingBlockBase {
	...
	height = Device.system.phone ? "75vh" : "100%";
		...
		if (this._chart) {
			...
			this._actions = this.chartActions
				? this.getToolbarActions(
						this._chartContext,
						this.chartDefinition?.isInsightsEnabled,
						this.chartDefinition?.isInsightsVisible
				  )
				: "";
		} else {
	...
	static checkChartVisualizationPath = (visualizations: Record<string, string>[], visualizationPath: string | undefined) => {
		visualizations.forEach(function (visualization: Record<string, string>) {
			if (visualization.$AnnotationPath.includes(`@${UIAnnotationTerms.Chart}`)) {
	...
	getToolbarActions = (
		...
		if (isInsightsEnabled) {
			actions.push(this.getStandardActions(isInsightsEnabled, isInsightsVisible));
		}
		if (actions.length > 0) {
	...
	getStandardActions = (
		...
		return xml`<mdcat:ActionToolbarAction  xmlns="sap.m" xmlns:mdcat="sap.ui.mdc.actiontoolbar" visible="${isInsightsVisible}">
				<Button
					id="${generate([this.id, "StandardAction::Insights"])}"
		...
		for (const action of actions) {
			if (action.menu) {
				action.menu.forEach((item) => {
					const idx = actions.map((act) => act.key).indexOf((item as BaseAction).key);
					if (idx !== -1) {
