			...
			if (aModes && aModes.includes("Filter")) {
				...
				if (aKnownAggregatableProps.includes(sKey) && !aGroupablePropertiesValues.includes(sKey)) {
					aProperties = addPropertyToChart(
						...
						sKey,
						oPropertyAnnotations,
						customAggregates.includes(sKey) ? false : oFilterRestrictionsInfo?.propertyInfo?.[sKey]?.filterable ?? true,
						oFilterRestrictionsInfo,
						sortRestrictionsInfo,
						oMDCChart,
						sCriticality,
						oObj,
						false,
						customAggregates.includes(sKey) ? false : true,
						undefined,
						true
					);
				}
...
function addPropertyToChart(
	...
	oFilterRestrictionsInfo: FilterRestrictionsInfoType,
	...
	aProperties.push({
		...
		maxConditions: isMultiValueFilterExpression(oFilterRestrictionsInfo.propertyInfo[sKey] as unknown as string) ? -1 : 1,
		sortKey: sKey,
		path: sKey,
...
function _getChartPropertyTypeConfig(oMDCChart: Chart, filterable: boolean, oObj: MetaModelNavProperty): TypeConfig | undefined {
	let typeConfig;
	if (filterable) {
		typeConfig = oMDCChart?.getTypeMap()?.getTypeConfig(oObj.$Type);
	}
	return typeConfig;
}
...
ChartDelegate.updateBindingInfo = function (oChart: Chart, oBindingInfo: BindingInfo): void {
