			...
			for (const sProperty in mConditions) {
				...
				if (
					...
					(((!mTargetProperties[sProperty] || // no target property found by property name
						...
						(!bIsFilterBarEntityType || (sProperty === "$editState" && bIsChart))) || //type does not match OR $editState on secondary entity set
		...
		filterPropertiesMetadata.forEach(function (filterProperty) {
			...
			if (
				...
				((!filterProperty.isCustomFilter && filterProperty.dataType !== entityPropertiesCurrent.dataType) ||
					(filterProperty.isCustomFilter &&
						!entityProperties.find(
							(property) => !property.isCustomFilter && property.annotationPath == filterProperty.annotationPath
						)))
			) {
