...
function _templateEditState(sIdPrefix: any, metaModel: ODataMetaModel, oModifier: any) {
	...
	return DelegateUtil.templateControlFragment("sap.fe.core.filter.DraftEditState", oPreprocessorSettings, undefined, oModifier).finally(
		function () {
			oThis.destroy();
		}
	);
...
ODataFilterBarDelegate.clearFilters = async function (oFilterControl: unknown) {
	const filterBarAPI = (oFilterControl as FilterBar).getParent();
	return (filterBarAPI as FilterBarAPI).clearFilterValues(oFilterControl as FilterBar);
};
/**
...
ODataFilterBarDelegate.fetchProperties = async function (oFilterBar: FilterBar): Promise<FilterFieldPropertyInfo[]> {
...
ODataFilterBarDelegate.getTypeMap = function (): object {
	return TypeMap;
};

export type FilterFieldPropertyInfo = PropertyInfo & {
