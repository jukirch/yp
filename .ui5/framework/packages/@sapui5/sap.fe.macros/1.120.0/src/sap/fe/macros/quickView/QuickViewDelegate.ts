...
SimpleLinkDelegate.fnTemplateFragment = async function (link: Link): Promise<Element> {
 ...
 */
SimpleLinkDelegate.fetchAdditionalContent = async function (oMdcLinkControl: Link): Promise<Control[]> {
	const oPayLoad = oMdcLinkControl.getPayload() as SimpleLinkDelegatePayload;
	...
	const oBindingContext =
		aNavigateRegexpMatch && aNavigateRegexpMatch.length > 1 && aNavigateRegexpMatch[1]
			? oMdcLinkControl
					.getModel()
					?.bindContext(aNavigateRegexpMatch[1], oMdcLinkControl.getBindingContext() || undefined, { $$ownRequest: true })
			: null;
	...
	if (oMdcLinkControl && oMdcLinkControl.isA(CONSTANTS.sapuimdcLink)) {
		...
		return this.fnTemplateFragment(oMdcLinkControl).then(function (oPopoverContent: Control | Control[]) {
			if (oBindingContext) {
				const boundContext = oBindingContext.getBoundContext();
				if (boundContext) {
					(oPopoverContent as Control).setBindingContext(boundContext);
				}
			}
			return [oPopoverContent];
		});
...
SimpleLinkDelegate._fetchLinkCustomData = function (_oLink: Link): CustomData[] | undefined {
	if (
		_oLink.getParent() &&
 ...
 */
SimpleLinkDelegate.fetchLinkItems = async function (oLink: Link, oBindingContext: Context, oInfoLog: InfoLog): Promise<object | null> {
 ...
 */
SimpleLinkDelegate.fetchLinkType = async function (oLink: FELink): Promise<object | undefined> {
	const _oCurrentLink = oLink;
	const _oPayload = Object.assign({}, oLink.getPayload()) as SimpleLinkDelegatePayload;
...
SimpleLinkDelegate._getLinkItemWithNewParameter = async function (
	...
	return _oShellServices.expandCompactHash(_oLinkItem.getHref()).then(async function (sHash: any) {
		...
		const titleIntent = _oPayload.semanticPrimaryActions[_oPayload.semanticObjects.indexOf(_oPayload.mainSemanticObject)]?.intent ?? "";
...
SimpleLinkDelegate.modifyLinkItems = async function (
	...
	if (aLinkItems.length !== 0) {
		...
		let primaryActionIsActive = (await FieldHelper.checkPrimaryActions(payloadResolved, true, oAppComponent)) as {
			titleLink: Partial<{ intent: string }>;
			hasTitleLink: boolean;
		};
		let aTitleLink = primaryActionIsActive.titleLink;
		let bTitleHasLink: boolean = primaryActionIsActive.hasTitleLink;
		...
		try {
			...
			if (SimpleLinkDelegate._IsSemanticObjectDynamic(aNewLinkCustomData, this)) {
				...
				primaryActionIsActive = (await FieldHelper.checkPrimaryActions(oPayloadResolved, true, oAppComponent)) as {
					titleLink: Partial<{ intent: string }>;
					hasTitleLink: boolean;
				};
...
SimpleLinkDelegate._createNewPayloadWithDynamicSemanticObjectsResolved = function (
	...
	const navigationPath = payload.navigationPath?.match(/{(.*?)}/);
	for (const i in payload.semanticObjects) {
		...
		if (SimpleLinkDelegate._isDynamicPath(semanticObjectName)) {
			...
			if (!semanticObjectsResolved[tmpPropertyName] && payload.navigationPath) {
				if (navigationPath?.length && navigationPath.length > 1) {
 ...
 */
SimpleLinkDelegate.calculatePayloadWithDynamicSemanticObjectsResolved = function (link: Link): RegisteredPayload {
...
SimpleLinkDelegate._calculateSemanticAttributes = function (
	oContextObject: Record<string, string>,
	oPayload: RegisteredPayload,
	oInfoLog: InfoLog,
	oLink: Link
): { payload: RegisteredPayload; results: Record<string, string> } {
