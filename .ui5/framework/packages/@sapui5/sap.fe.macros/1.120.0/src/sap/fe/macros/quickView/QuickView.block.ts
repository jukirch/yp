import type { AnnotationTerm, EntityType, NavigationProperty, Property } from "@sap-ux/vocabularies-types";
...
export default class QuickViewBlock extends BuildingBlockBase {
	...
	private valueDataModelPath!: DataModelObjectPath;
	...
	writeCustomData(semanticObjects: string[], valueDataModelPath: DataModelObjectPath | undefined): string {
		let customData = "";
		...
		for (const semanticObject of semanticObjects) {
			...
			if (dynamicPathFromSemanticObject) {
				const dynamicPathWithRelativeLocation = compileExpression(
