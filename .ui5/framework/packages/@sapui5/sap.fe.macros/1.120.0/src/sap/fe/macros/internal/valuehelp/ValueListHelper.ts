...
import type FilterBar from "sap/ui/mdc/filterbar/vh/FilterBar";
...
const ValueListHelper = {
	 ...
	 */
	getColumnWidth: function (propertyPath: DataModelObjectPath): string | undefined {
	...
	showValueList: async function (payload: ValueHelpPayload, container: Container, selectedContentId: string): Promise<void> {
		const valueHelp = container.getParent() as ValueHelp,
			...
			metaModel = (valueHelp.getControl()?.getModel() as ODataModel).getMetaModel(),
			vhUIModel =
