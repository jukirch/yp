...
sap.ui.define(["../../../UIUtil", "../../core/errors", "../../sina/ComparisonOperator", "../../sina/DataSource", "../../sina/DataSourceType", "../../sina/SearchResultSetItemAttribute", "../../sina/SimpleCondition", "../../sina/UserCategoryDataSource", "../AbstractProvider", "./template", "./template2"], function (_____UIUtil, ____core_errors, ____sina_ComparisonOperator, ____sina_DataSource, ____sina_DataSourceType, ____sina_SearchResultSetItemAttribute, ____sina_SimpleCondition, ____sina_UserCategoryDataSource, ___AbstractProvider, ___template, ___template2) {
    ...
    _createClass(Provider, [{
      ...
      value: function adjustHighlights(items, searchTerm) {
        ...
        for (var i = 0; i < items.length; i++) {
          ...
          for (var j = 0; j < items[i].detailAttributes.length; j++) {
            var detailAttr = items[i].detailAttributes[j];
          ...
          for (var k = 0; k < items[i].titleAttributes.length; k++) {
            var titleAttr = items[i].titleAttributes[k];
      ...
      value: function executeSearchQuery(searchQuery) {
        try {
          ...
          return _await(new Promise(function (resolve) {
            ...
            var items;
            var eshGptData = Provider.getEshGptResponseData(dataSourceId, searchTerm);
            if (_this2.searchQuery.eshGpt && Provider.isSupportedEshGptAllQuery(dataSourceId, searchTerm)) {
              searchTerm = eshGptData.searchTerm;
              dataSourceId = eshGptData.dataSource.id;
              dataSourceType = eshGptData.dataSource.type;
            }
            if (dataSourceId === Provider.dataSourceIds.Scientists || dataSourceId === Provider.dataSourceIds.Folklorists) {
              ...
              if (eshGptData.dataSource) {
                searchQuery.filter.dataSource = eshGptData.dataSource;
              }
              ...
              resultSet = _this2.sina._createSearchResultSet({
                ...
                facets: facets1,
                query: searchQuery,
                nlqSuccess: eshGptData.nlqSuccess,
                title: eshGptData.title,
                totalCount: items.length
              });
            } else if (dataSourceId === Provider.dataSourceIds.Mysterious_Sightings || dataSourceId === Provider.dataSourceIds.Urban_Legends || _this2.searchQuery.eshGpt && Provider.isSupportedEshGptAllQuery(dataSourceId, searchTerm)) {
            ...
            } else if (dataSourceId === "Publications" || _this2.searchQuery.eshGpt && Provider.isSupportedEshGptAllQuery(dataSourceId, searchTerm)) {
            ...
            } else if (dataSourceId === Provider.dataSourceIds.All || dataSourceType === _this2.sina.DataSourceType.UserCategory) {
      ...
      value: function generateFacets(searchQuery) {
        if (searchQuery.filter.dataSource.id === Provider.dataSourceIds.All || searchQuery.filter.dataSource.type === this.sina.DataSourceType.UserCategory) {
      ...
      key: "isSupportedEshGptAllQuery",
      value: function isSupportedEshGptAllQuery(dataSourceId, searchTerm) {
        // sample provider only supports a few dedicated search terms (see <query>.eshGpt)
        if (dataSourceId === Provider.dataSourceIds.All) {
          return searchTerm.toLowerCase() === "all scientists" || searchTerm.toLowerCase() === "all folklorists" || searchTerm.toLowerCase() === "all publications" || searchTerm.toLowerCase() === "all urban legends" || searchTerm.toLowerCase() === "all mysterious sightings";
        }
        return false;
      }
    }, {
      key: "getEshGptResponseData",
      value: function getEshGptResponseData(dataSourceId, searchTerm) {
        var eshGptData = {
          nlqSuccess: false,
          title: ""
        };
        if (Provider.isSupportedEshGptAllQuery(dataSourceId, searchTerm)) {
          if (dataSourceId === Provider.dataSourceIds.All) {
            if (searchTerm.toLowerCase() === "all scientists") {
              eshGptData = {
                dataSource: new DataSource({
                  type: DataSourceType.BusinessObject,
                  id: Provider.dataSourceIds.Scientists,
                  label: "Scientist",
                ...
                title: JSON.stringify({
                  scope: Provider.dataSourceIds.Scientists,
                  query: "*"
                })
              };
            } else if (searchTerm.toLowerCase() === "all folklorists") {
              eshGptData = {
                dataSource: new DataSource({
                  type: DataSourceType.BusinessObject,
                  id: Provider.dataSourceIds.Folklorists,
                  label: "Folklorist",
                ...
                nlqSuccess: true,
                title: JSON.stringify({
                  scope: Provider.dataSourceIds.Folklorists,
                  query: "*"
                })
              };
            } else if (searchTerm.toLowerCase() === "all publications") {
              eshGptData = {
                dataSource: new DataSource({
                  type: DataSourceType.BusinessObject,
                  id: Provider.dataSourceIds.Publications,
                  label: "Publication",
                ...
                nlqSuccess: true,
                title: JSON.stringify({
                  scope: Provider.dataSourceIds.Publications,
                  query: "*"
                })
              };
            } else if (searchTerm.toLowerCase() === "all urban legends") {
              eshGptData = {
                dataSource: new DataSource({
                  type: DataSourceType.BusinessObject,
                  id: Provider.dataSourceIds.Urban_Legends,
                  label: "Urban Legend",
                ...
                nlqSuccess: true,
                title: JSON.stringify({
                  scope: Provider.dataSourceIds.Urban_Legends,
                  query: "*"
                })
              };
            } else if (searchTerm.toLowerCase() === "all mysterious sightings") {
              eshGptData = {
                dataSource: new DataSource({
                  type: DataSourceType.BusinessObject,
                  id: Provider.dataSourceIds.Mysterious_Sightings,
                ...
                nlqSuccess: true,
                title: JSON.stringify({
                  scope: Provider.dataSourceIds.Mysterious_Sightings,
                  query: "*"
                })
              };
            }
  ...
  _defineProperty(Provider, "dataSourceIds", {
    All: "All",
