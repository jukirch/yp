...
(function(){
sap.ui.define(["./i18n", "sap/m/TablePersoController", "./SearchResultTableColumnType", "./error/errors", "sap/m/library"], function (__i18n, TablePersoController, ___SearchResultTableColumnType, ___error_errors, sap_m_library) {
  ...
  var ResetAllMode = sap_m_library["ResetAllMode"];
  var TablePersoColumnIdPrefix;
  (function (TablePersoColumnIdPrefix) {
    TablePersoColumnIdPrefix["TITLE"] = "TABLE_COLUMN_TITLE";
    TablePersoColumnIdPrefix["TITLE_DESCRIPTION"] = "TABLE_COLUMN_TITLE_DESCRIPTION";
    TablePersoColumnIdPrefix["DETAIL"] = "TABLE_COLUMN_DETAIL_";
    TablePersoColumnIdPrefix["RELATED_APPS"] = "TABLE_COLUMN_RELATED_APPS";
    TablePersoColumnIdPrefix["EXTEND"] = "TABLE_COLUMN_EXTEND";
  })(TablePersoColumnIdPrefix || (TablePersoColumnIdPrefix = {}));
    ...
    _createClass(SearchResultTablePersonalizer, [{
      ...
      value: function update(searchResultTable) {
        ...
        try {
          ...
          this.setStorageId("search-result-table-state-" + this.model.getDataSource().id);
      ...
      key: "setStorageId",
      value: function setStorageId(id) {
        this.storageId = id;
      }
    }, {
      key: "getStorageId",
      ...
      key: "getStorageIdOfInitialState",
      value: function getStorageIdOfInitialState() {
        return this.storageId + "INITIAL";
      }
    }, {
      ...
      value: function updatePersoController() {
        ...
        try {
          ...
          // this.persoController.getTablePersoDialog().attachConfirm(function () {
          //     let hasColumnVisible = false;
          //     for (const columnId in this.getPersoMap()) {
          //         const columnData = this.getPersoMap()[columnId];
          //         if (
          //             typeof columnData === "object" &&
          //             typeof columnData.getVisible === "function" &&
          //             columnData.getVisible() === true
          //         ) {
      ...
      value: function isValid(persoState) {
        ...
        // perso state has old version column id (attribute id) -> NOT valid
        var hasOutdatedColumnId = false;
        for (var i = 0; i < persoState.aColumns.length; i++) {
          if (!this.isPeroColumnIdValid(persoState.aColumns[i])) {
            hasOutdatedColumnId = true;
            break;
          }
        }
        if (hasOutdatedColumnId) {
          return false;
        }
        // perso state has no column visible -> NOT valid
        var hasColumnVisible = false;
        for (var j = 0; j < persoState.aColumns.length; j++) {
          if (persoState.aColumns[j].visible === true) {
            hasColumnVisible = true;
            break;
          }
        }
      ...
      key: "isPeroColumnIdValid",
      value: function isPeroColumnIdValid(persoColumn) {
        var isValid = false;
        for (var key in TablePersoColumnIdPrefix) {
          if (persoColumn.id.includes(TablePersoColumnIdPrefix[key])) {
            isValid = true;
            break;
          }
        }
  ...
  SearchResultTablePersonalizer.TablePersoColumnIdPrefix = TablePersoColumnIdPrefix;
  return SearchResultTablePersonalizer;
});
})();