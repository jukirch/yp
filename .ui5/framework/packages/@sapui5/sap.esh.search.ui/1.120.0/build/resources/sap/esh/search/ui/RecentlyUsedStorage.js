...
sap.ui.define(["sap/base/Log", "./i18n", "./SearchHelper", "./sinaNexTS/sina/ComplexCondition", "./sinaNexTS/sina/HierarchyDisplayType", "./suggestions/SuggestionType"], function (Log, __i18n, ___SearchHelper, ___sinaNexTS_sina_ComplexCondition, ___sinaNexTS_sina_HierarchyDisplayType, ___suggestions_SuggestionType) {
  ...
  var SearchHelper = ___SearchHelper;
  var RecentlyUsedStorage = /*#__PURE__*/function () {
    ...
    _createClass(RecentlyUsedStorage, [{
      ...
      value: function assembleLabel(searchTerm, dataSource) {
        ...
        var hierarchyNodePathLength = this.searchModel.getProperty("/breadcrumbsHierarchyNodePaths").length;
        if (hierarchyNodePathLength > 0) {
          var breadCrumb = "";
          var pathSep = " > ";
          var hierarchyNodes = this.searchModel.getProperty("/breadcrumbsHierarchyNodePaths").slice();
          if (document.documentElement.getAttribute("dir") === "rtl") {
            pathSep = " < ";
            hierarchyNodes = hierarchyNodes.reverse();
          }
          hierarchyNodes.forEach(function (hierarchyNode, index) {
            breadCrumb += hierarchyNode.label;
            if (++index < hierarchyNodePathLength) {
              breadCrumb += pathSep;
            }
          });
          return i18n.getText("resultsIn", ["<span>" + searchTerm + "</span>", breadCrumb]);
        }
      ...
      value: function getItems() {
        ...
        var items = (_this$personalization = this.personalizationStorage.getItem("ESH-Recent-Searches")) !== null && _this$personalization !== void 0 ? _this$personalization : [];
        items = items.filter(function (item) {
          return (
            // do not return item which is equal to current query
            item.url !== SearchHelper.getHashFromUrl()
          );
        });
        return items;
      }
