...
sap.ui.define(["../core/core", "./Query", "./EqualsMode", "./ConditionType", "./DataSourceType", "../core/errors", "./ComparisonOperator"], function (core, ___Query, ___EqualsMode, ___ConditionType, ___DataSourceType, ___core_errors, ___ComparisonOperator) {
  ...
  var SearchQuery = /*#__PURE__*/function (_Query) {
    ...
    function SearchQuery(properties) {
      var _properties$calculate, _properties$multiSele, _properties$nlq, _properties$eshGpt, _properties$facetTop, _properties$groupBy;
      var _this;
      ...
      _defineProperty(_assertThisInitialized(_this), "eshGpt", false);
      ...
      _this.eshGpt = (_properties$eshGpt = properties.eshGpt) !== null && _properties$eshGpt !== void 0 ? _properties$eshGpt : _this.eshGpt;
    ...
    _createClass(SearchQuery, [{
      ...
      key: "setEshGpt",
      value: function setEshGpt() {
        var eshGpt = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
        this.eshGpt = eshGpt;
      }
    }, {
      key: "setFacetTop",
      ...
      value: function _executeSearchQueryInFolderMode(query) {
        try {
          var _await$Promise$all, _await$Promise$all2, resultSetForFacets, resultSetForItems;
          const _this5 = this;
          // execute queries
          var queryForFacets = query; // query with descendant-of filter for getting the facets
          var queryForItems = _this5._assembleQueryForItems(query); // query with child-of filter for getting the result set items
          return _await(_this5._doExecuteSearchQuery(queryForFacets), function (_this5$_doExecuteSear) {
            return _await(_this5._doExecuteSearchQuery(queryForItems), function (_this5$_doExecuteSear2) {
              return _await(Promise.all([_this5$_doExecuteSear, _this5$_doExecuteSear2]), function (_Promise$all) {
                ...
                resultSetForFacets = _await$Promise$all2[0];
                resultSetForItems = _await$Promise$all2[1];
                var mergedResultSet = _this5._mergeResultSetsInNavigationFolderMode(resultSetForFacets, resultSetForItems);
      ...
      value: function _mergeResultSetsInNavigationFolderMode(resultSetForFacets, resultSetForItems) {
        // move items from resultSetForItems to resultSetForFacets
        resultSetForFacets.items = [];
        var _iterator = _createForOfIteratorHelper(resultSetForItems.items),
          _step;
        try {
      ...
      key: "_assembleQueryForItems",
      value: function _assembleQueryForItems(query) {
        // clone query
        var queryForItems = query.clone();
        queryForItems.calculateFacets = false;
        var folderAttribute = queryForItems.filter.getFolderAttribute();
        ...
        var folderAttributeConditions = queryForItems.filter.rootCondition.getConditionsByAttribute(folderAttribute);
        var _iterator2 = _createForOfIteratorHelper(folderAttributeConditions),
          _step2;
        try {
      ...
      key: "isAdditionalRootNodeFilterNeeded",
      value: function isAdditionalRootNodeFilterNeeded(query) {
