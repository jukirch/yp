...
sap.ui.define([
     ...
     * <code>sap.ushell.Container.getServiceAsync("PluginManager")</code>. For details, see
    ...
    function PluginManager (oContainerInterface, sParameter, oServiceProperties) {
        ...
        this.registerPlugins = function (oPlugins) {
            ...
            Object.keys(oPlugins).sort().forEach(function (sPluginName) {
                var oCurrentPlugin = oPlugins[sPluginName] || {};
                var oPluginConfig = oCurrentPlugin.config || {};
                var sPluginCategory = oPluginConfig[S_PLUGIN_TYPE_PARAMETER] || "";
                ...
                if (that._oConfig.isBlueBox === true) {
                    if (oCurrentPlugin.config && oCurrentPlugin.config["sap-plugin-agent"] === true) {
                        var sSapAgentId = (oCurrentPlugin.config["sap-plugin-agent-id"] || sPluginName);
        ...
        this._isFormFactorSupported = function (oPlugin) {
            ...
            var sCurrentFormFactor = utils.getFormFactor();
