...
sap.ui.define([
    ...
    var FLPScheduler = {
        ...
        _adaptForSpecialEvents: function (values) {
            var oStepConfig = values[0];
            if (oStepConfig.ConditionalWaitForAppLoading &&
                oStepConfig.ConditionalWaitForAppLoading.continueOnEvent &&
                oStepConfig.ConditionalWaitForAppLoading.continueOnEvent.eventName === "startUpFesrEnhanced" &&
                FESR.getActive() === false) {
                // To not change loading behaviour for not enabled FESR ABAP-Servers, the default event timeout is overridden.
                oStepConfig["ConditionalWaitForAppLoading"].continueOnEvent.timeoutInMs = 3000;
            }
        },
