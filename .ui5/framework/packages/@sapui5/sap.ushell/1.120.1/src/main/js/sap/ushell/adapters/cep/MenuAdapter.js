...
sap.ui.define([
    ...
    MenuAdapter.prototype._buildMenuEntries = function (aSpaces) {
        ...
        return aSpaces
            ...
            .map(function (oSpace) {
                var oTopMenuEntry = {
                    title: oSpace.descriptor.value.title || oSpace.id,
                ...
                var aSubMenuEntries = oSpace.workPages.nodes.map(function (oPage) {
                    oPage.contents = oPage.contents || {};
                    oPage.contents.descriptor = oPage.contents.descriptor || {};
                    oPage.contents.descriptor.value = oPage.contents.descriptor.value || {};
    ...
    MenuAdapter.prototype._buildContentNodes = function (aSpaces) {
        return aSpaces
            ...
            .map(function (oSpace) {
                return {
                    id: oSpace.id,
                    label: oSpace.descriptor.value.title || oSpace.id,
                    type: oContentNodeTypes.Space,
                    isContainer: false,
                    children: oSpace.workPages.nodes.map(function (oPage) {
                        return {
                            id: oPage.id,
                            label: oPage.contents.descriptor.value.title || oPage.id,
                            type: oContentNodeTypes.Page,
                            isContainer: this._isTraditionalPage(oPage.contents.descriptor),
                            children: []
                        };
                    }.bind(this))
                };
    ...
    MenuAdapter.prototype.isWorkPage = function (sPageId) {
        return this.oSiteDataPromise
            ...
            .then(function (aSpaces) {
                ...
                if (oFoundPage) {
                    return !this._isTraditionalPage(oFoundPage.contents.descriptor);
                }
            ...
            "){" +
                "nodes{" +
                    ...
                    "workPages{" +
                        "nodes{" +
                            "id," +
                            "contents(queryInput:{selectProperties:[\"/descriptor\"],selectDescriptorProperties:[\"/value/title\",\"/value/pageType\"]})" +
                        "}" +
                    "}" +
                "}" +
            "}" +
