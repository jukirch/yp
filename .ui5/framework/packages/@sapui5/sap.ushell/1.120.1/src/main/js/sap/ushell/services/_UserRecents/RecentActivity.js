...
sap.ui.define([
    ...
    RecentActivity.prototype.newItem = function (oItem) {
        ...
        const sIcon = this.getActivityIcon(oItem.appType, oItem.icon);
        let bAlreadyIn;
                    ...
                    for (activityIndex = 0; activityIndex < this.oRecentActivities.recentUsageArray.length && !bIsResolved; activityIndex++) {
                        ...
                        if (oActivity[sCurrentDevice] === undefined) {
                            ...
                            if (!(oActivity.oItem.url[0] === "#")) {
                                ...
                                const sShellHash = oActivity.oItem.appId.split("#")[1];
                                oIntent.target.shellHash = sShellHash + sMandatoryParams;
                                aIntentsToResolve.push(oIntent);
                            } else {
                                const sShellHash = oActivity.oItem.appId.split("#")[1];
                                oIntent.target.shellHash = sShellHash;
                                aIntentsToResolve.push(oIntent);
                    ...
                    if (aIntentsToResolve.length > 0) {
                        sap.ushell.Container.getServiceAsync("CrossApplicationNavigation")
                            .then(CrossApplicationNavigationService => {
                                ...
                                CrossApplicationNavigationService.isNavigationSupported(aIntentsToResolve)
                                    .done(aResolved => {
                                        ...
                                        for (activityIndex = 0; activityIndex < this.oRecentActivities.recentUsageArray.length && !bIsResolved; activityIndex++) {
                                            ...
                                            if (oActivity[sCurrentDevice] === undefined) {
                                                ...
                                                var oItem = aResolved[activityIndex];
    ...
    RecentActivity.prototype.getRecentItems = function () {
        return new Promise((resolve, reject) => {
            this.getRecentItemsHelper(RecentActivity.ITEM_COUNT)
                .then(recentItems => {
                    resolve(recentItems.map(oRecentEntry => oRecentEntry.oItem));
                })
                .catch(errorMessage => {
                    reject(errorMessage);
                });
        ...
        return new Promise((resolve, reject) => {
            this.getRecentItemsHelper()
                .then(recentItems => {
                    ...
                    resolve(aFrequentActivity.map(oRecentEntry => oRecentEntry.oItem));
                })
                .catch(errorMessage => {
                    reject(errorMessage);
                });
