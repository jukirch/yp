...
sap.ui.define([
    ...
    return UIComponent.extend("sap.ushell.components.workPageBuilder.Component", /** @lends sap.ushell.components.workPageBuilder.Component */{
         ...
         *
         * @returns {Promise<sap.ushell.services.VisualizationInstantiation>} A promise resolving to the VizInstantiation service.
         * @since 1.112.0
         * @private
         */
        ...
        */
        setVisualizationData: function (oVizNodes) {
            return this._transformVizData(ObjectPath.get("visualizations.nodes", oVizNodes) || []).then(function (aPreparedVizData) {
                this.getRootControl().getController().setVisualizationData({
                    visualizations: {
                        nodes: aPreparedVizData,
                        totalCount: ObjectPath.get("visualizations.totalCount", oVizNodes) || 0
                    }
                });
            }.bind(this));
        },
        ...
        setVisualizationDataPaginated: function (oVizNodes) {
            return this._transformVizData(ObjectPath.get("visualizations.nodes", oVizNodes) || []).then(function (aPreparedVizData) {
                this.getRootControl().getController().setVisualizationDataPaginated({
                    visualizations: {
                        nodes: aPreparedVizData,
                        totalCount: ObjectPath.get("visualizations.totalCount", oVizNodes)
                    }
                });
            }.bind(this));
        },
        ...
        */
         ...
         * @returns {Promise<object[]>} The modified visualization data.
         * @since 1.110.0
         * @private
         */
         ...
         * @returns {Promise<object[]>} A promise resolving with the modified vizData.
         * @since 1.110.0
         * @private
         */
                        ...
                        if (sContentProviderId && oIndicatorDataSource && oIndicatorDataSource.path) {
                            return oCstrService.getSystemContext(sContentProviderId).then(function (oSystemContext) {
                                if (oSystemContext) {
                                    ...
                                    var oDataSource = ObjectPath.get(["descriptor", "value", "sap.app", "dataSources", sDataSourceId], oVizData);
                                    var sFullyQualifiedXhrUrl;

                                    if (oDataSource) {
                                        sFullyQualifiedXhrUrl = oSystemContext.getFullyQualifiedXhrUrl(oDataSource.uri);
                                        ObjectPath.set(["descriptor", "value", "sap.app", "dataSources", sDataSourceId, "uri"],
                                            sFullyQualifiedXhrUrl, oVizData);
                                    } else {
                                        sFullyQualifiedXhrUrl = oSystemContext.getFullyQualifiedXhrUrl(oIndicatorDataSource.path);
                                        ObjectPath.set(["descriptor", "value", "sap.flp", "indicatorDataSource", "path"], sFullyQualifiedXhrUrl, oVizData);
                                    }
                                }
                                return oVizData;
         ...
         * @returns {Promise} A promise resolving when the data has been saved.
         * @since 1.110.0
         * @private
         */
         ...
         * @returns {object} The application descriptor.
         */
         ...
         * @returns {object} The vizType definition object.
         */
