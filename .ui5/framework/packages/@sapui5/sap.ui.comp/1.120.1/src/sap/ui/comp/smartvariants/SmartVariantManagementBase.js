...
sap.ui.define([
	 ...
	 * @returns {sap.ui.comp.smartvariants.SmartVariantManagement} the current instance
	 * @private
	 * @restricted sap.ui.comp
 	 */
	SmartVariantManagementBase.prototype.setShowAsText = function(bValue) {
	 ...
	 * @restricted sap.ui.comp
 	 */
	SmartVariantManagementBase.prototype.getShowAsText = function() {
	...
	SmartVariantManagementBase.prototype.updateVariant = function (oVariant) {
		...
		if (oVariant) {
			...
			if (oVariantItem) {
				...
				oVariantItem._setOriginalExecuteOnSelect(oVariant.getExecuteOnSelection());

				oVariantItem.setFavorite(oVariant.getFavorite());
				oVariantItem._setOriginalFavorite(oVariant.getFavorite());

				oVariantItem.setTitle(oVariant.getText("variantName"));
				oVariantItem._setOriginalTitle(oVariant.getText("variantName"));
				oVariantItem.setText(oVariant.getText("variantName"));

				if (oVariant.getContexts) {
					oVariantItem.setContexts(oVariant.getContexts());
					oVariantItem._setOriginalContexts(oVariant.getContexts());
				}
			}
		}
	};
	...
	SmartVariantManagementBase.prototype._reapplyExecuteOnSelectForStandardVariantItem = function (bSelect) {
		...
		if (this._oVM) {
			var oItem = this._oVM.getItemByKey(sStdKey);
			if (oItem) {
				oItem.setExecuteOnSelect(bSelect);
				oItem._setOriginalExecuteOnSelect(bSelect);
			}
		}
	};
