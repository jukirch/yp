...
sap.ui.define([
	...
	ODataControlFactory.prototype._init = function(oMetaData) {
		...
		if (this._oModel) {
			...
			if (!oEdmProperty && oFunctionImport) {
				oEdmProperty = this._findProperty(oFunctionImport, oMetaData.path);
				if (oEdmProperty) {
	...
	ODataControlFactory.prototype._checkTextInDisplayModeValueList = function (oSettings) {
		...
		return !!(
			...
			!(this._oParent.getExpandNavigationProperties && this._oParent.getExpandNavigationProperties()) && // There is no navigation property
	...
	ODataControlFactory.prototype.bind = function(oSettings) {
		...
		if (oConfig && oConfig.configdata) {
			try {
				...
				if (this._oMetaData && this._oMetaData.annotations && oConfig.configdataVL) {
