...
sap.ui.define([
	...
	VariantConverterTo.addP13nRanges = function(aOutputRanges, aRanges, sFieldType, bUTC) {
		var sSign, sOption, sLow, sHigh;
		...
		for (var i = 0; i < aRanges.length; i++) {
			...
			if (["date", "datetime", "time"].includes(sFieldType)) {
				sLow = VariantConverterTo._processP13NDateValue(aRanges[i].value1, sFieldType, bUTC);
				sHigh = VariantConverterTo._processP13NDateValue(aRanges[i].value2, sFieldType, bUTC);
			}
	...
	VariantConverterTo._processP13NDateValue = function (vValue, sFieldType, bUTC) {
		if (!vValue) {
			return vValue;
		}
