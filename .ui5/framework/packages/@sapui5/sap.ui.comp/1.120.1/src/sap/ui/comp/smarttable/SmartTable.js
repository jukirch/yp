...
sap.ui.define([
	...
	var SmartTable = VBox.extend("sap.ui.comp.smarttable.SmartTable", /** @lends sap.ui.comp.smarttable.SmartTable.prototype */ {
		metadata: {
			...
			events: {
				 ...
				 * @param {sap.ui.base.EventProvider} oControlEvent.getSource
				 * @param {object} oControlEvent.getParameters
				 * @param {object[]} oControlEvent.getParameters.columnInfos Pasting-related column information
				 * @param {boolean} oControlEvent.getParameters.columnInfos.property Data property that is used when pasting data
				 * @param {boolean} oControlEvent.getParameters.columnInfos.ignore Defines whether the column will be ignored during pasting
				 * @param {sap.ui.model.SimpleType} oControlEvent.getParameters.columnInfos.type The instance that is used to parse the pasted value
				 * @param {function} oControlEvent.getParameters.columnInfos.customParseFunction Function that is used to parse the pasted value when no type instance is available (for example, for custom columns)
				 *
	...
	SmartTable.prototype.setHeaderStyle = function(sStyle) {
		...
		this._headerText && this._headerText.setTitleStyle(sStyle || TitleLevel.H4);
		return this;
	};
	...
	SmartTable.prototype._addCopyButton = function() {
		...
		var oCopyButton = Core.byId(sCopyButtonId);
		if (oCopyButton) {
	...
	SmartTable.prototype._addHeaderToToolbar = function() {
		...
		if (!this._headerText) {
			...
			this._headerText.setTitleStyle(this.getHeaderStyle() || TitleLevel.H4);
		}
	...
	SmartTable.prototype._triggerUI5ClientExport = function(bExportAs) {
		var that = this;
		var aSheetColumns, fnEventHandler, fnGetColumn, fnGetLabel, mExportSettings, oLibrary, oRowBinding;

		oLibrary = Core.loadLibrary("sap.ui.export", true);
		oLibrary.catch(function() {
			MessageBox.error(Core.getLibraryResourceBundle("sap.ui.comp").getText("SMARTTABLE_ERROR_MISSING_EXPORT_LIBRARY"));
		});
	...
	SmartTable.prototype._getExportColumnConfiguration = function(oColumn, bUtc) {
		...
		if (oP13nData.unit) {
			...
			} else if (oP13nData.displayBehaviour !== "idOnly") {
	...
	SmartTable.prototype._setExcelExportEnableState = function() {
		if (this._oExportButton) {
			this._oExportButton.setEnabled(!TableUtil.isEmpty(this._getRowBinding()));
		}
	};
	...
	SmartTable.prototype._createContent = function() {
		...
		if (!this._isMobileTable) {
			// Switch off default tooltips of the GridTable (incl. the column tooltips of the AnanlyticalTable) to ensure a
			// consistent handling of tooltips based on property useColumnLabelsAsTooltips
			this._oTable._setHideStandardTooltips(true);
		}
				...
				if (oChangeInfo && oChangeInfo.columns === "ModelChanged") {
					...
					if (this._isMobileTable && this._oTable.getHiddenInPopin()) {
						bEnableExpandedState = oEvent.getParameter("runtimeDeltaData").columns.columnsItems.some(function(oColumnItem) {
							...
							if (oColumn && oColumnItem.visible) {
								return this._oTable.getHiddenInPopin().includes(oColumn.getImportance());
							}
						}, this);
					}
	...
	SmartTable.prototype._adaptAdditionalProperties = function(oColumn) {
		...
		if (oColumnData.additionalProperty && oColumnData.additionalSortProperty) {
			var aAdditionalProperties = oColumnData.additionalProperty.split(',');
			var aSortProperties = oColumnData.additionalSortProperty.split(',');
		...
		if (sUseInfoToolbar === InfoToolbarBehavior.Off
			...
			if (iFiltersLength) {
				aFilters.forEach((oFilter) => {
					...
					oColumn.setFiltered && oColumn.setFiltered(true);
				});
			}
			return;
		}
	...
	SmartTable.prototype._getTablePersonalisationData = function() {
		...
		if (this._oCurrentVariant.filter) {
			this._oCurrentVariant.filter.filterItems.forEach(function(oModelItem) {
				...
				if (sPath) {
					try {
						var oMetadata = this._oTableProvider && this._oTableProvider.getFieldMetadata(sPath);
					} catch (e) {
						// do nothing
					}
					...
					if (bIsTimeField) {
						...
						if (oValue1 instanceof Date && !bIsTypeDateTimeOffset && this._oTableProvider && this._oTableProvider.getIsUTCDateHandlingEnabled()) {
							...
							if (oValue2) {
								...
								var iPrecision = parseInt(oMetadata && oMetadata.precision);
	...
	SmartTable.prototype._getColumnInfoForPaste = function() {
		var aVisibleColumnInfo = [], aInvisibleColumnInfo = [], aColumns = this._oTable.getColumns(), i, iLen = aColumns.length, oColumn, oColumnData, sPath, sType, oType, sAdditionalPath, bIgnore;
