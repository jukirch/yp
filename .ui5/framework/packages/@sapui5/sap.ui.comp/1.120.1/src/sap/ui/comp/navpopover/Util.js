...
sap.ui.define([
	...
	var Util = {
		...
		retrieveNavigationTargets: function(sSemanticObjectDefault, aAdditionalSemanticObjects, sAppStateKey, oComponent, oSemanticAttributes, sMainNavigationId, sPropertyName, oODataModel, sBindingPath, oLog) {
			...
			var oXApplNavigationPromise = Factory.getService("CrossApplicationNavigation", true);
			var oURLParsingPromise = Factory.getService("URLParsing", true);

			return Promise.all([oXApplNavigationPromise, oURLParsingPromise]).then(function(aValues) {
				var oXApplNavigation = aValues[0];
				var oURLParsing = aValues[1];
				...
				return new Promise(function(resolve) {
					...
					return oXApplNavigation.hrefForExternalAsync().then(function(sCurrentHash) {
		...
		navigate: function(sHref) {
			if (sHref.indexOf("#") === 0 && Factory.getUShellContainer()) {
				...
				if (!that.oNavigationPromise) {
					that.oNavigationPromise = Factory.getService("CrossApplicationNavigation", true).then(function (oCrossApplicationNavigation) {
