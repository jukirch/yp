...
sap.ui.define([
	...
	ValueHelpDialog.prototype.init = function() {
		...
		this._oCopyProvider = new CopyProvider({
			extractData: function (oRowContext, oColumn) {
				var oValue,
					sDisplayBehaviour = oColumn.data("fieldDisplayBehaviour"),
					vColumnFieldTemplate = oColumn.data("fieldTemplate"),
					sFieldName = oColumn.data("fieldName");

				if (!sFieldName && !vColumnFieldTemplate) {
					Log.error("Unable to extract data for copy - neither fieldName nor fieldTemplate is available.");
					return;
				}

				if (Array.isArray(vColumnFieldTemplate) && vColumnFieldTemplate.length > 1) {
					// TextArrangement handling
					if (sDisplayBehaviour === "descriptionAndId") {
						oValue = oRowContext.getProperty(vColumnFieldTemplate[1]) + " (" + oRowContext.getProperty(vColumnFieldTemplate[0]) + ")";
					} else {
						oValue = oRowContext.getProperty(vColumnFieldTemplate[0]) + " (" + oRowContext.getProperty(vColumnFieldTemplate[1]) + ")";
					}
				} else {
					oValue = oRowContext.getProperty(sFieldName);
				}
				return oColumn.__type ? oColumn.__type.formatValue(oValue, "string") : oValue;
			},
			copy: function(oEvent) {
				// some handling if necessary
			}
		});
			...
			this._oTablePromise = new Promise(function (fnResolve, fnReject) {
				...
				if (!this._isPhone()) {
					...
					if (this.getEnabledMultiSelectionPlugin()) {
						...
						oTable.addDependent(new MultiSelectionPlugin({limit: 1000, enableNotification: true})); // limited to 1000
	...
	ValueHelpDialog.prototype._initializeTableToolbar = function() {
		var oTableToolbar = this._oTable.getExtension()[0],
			...
			bSupportCopy = bTableSupportsCellSelector && this._bTableCreatedInternal;
		...
		if (!oTableTitle) {
			...
			if (bSupportCopy) {
				try {
					...
					this._oTable.addDependent(this._oCopyProvider);
					aExtensionContent.push(new ToolbarSpacer());
					aExtensionContent.push(
						new Button({
							icon: IconPool.getIconURI("copy"),
							tooltip: this._oRb.getText("VALUEHELPVALDLG_COPY_TOOLTIP"),
							press: function (oEvent) {
								this._oCopyProvider.copySelectionData(true);
							}.bind(this)
						})
					);
				} catch (oException) {
