...
export class SideEffectsService extends Service<SideEffectsSettings> {
	...
	public checkIfFieldIsRecommendationRelevant(field: Control): boolean {
		...
		if (context) {
			...
			const targetEntityTypeName = targetDataModelObject.entityType?.name || targetDataModelObject.targetType?.name;
			const recommendationRolesForEntity = this.recommendationRegistry.roles[targetEntityTypeName];
			if (recommendationRolesForEntity?.input?.includes(propertyName)) {
				return true;
			}
		}
		return false;
