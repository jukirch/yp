...
function parsePropertyValue(
	...
	if (annotationObject === null) {
		...
		value = { type: "Record", Record: parseAnnotationObject(annotationObject, currentTarget, annotationsLists, oCapabilities) };
	}
	...
	for (const annotationObjectKey in annotationObjects) {
		...
		switch (annotationKey) {
			...
			case `@com.sap.vocabularies.Common.v1.DraftRoot`:
				...
				if (
					annotationObjects[`@Org.OData.Capabilities.V1.FilterRestrictions`] &&
					annotationObjects[`@Org.OData.Capabilities.V1.FilterRestrictions`].FilterExpressionRestrictions?.length
				) {
					if (
						!annotationObjects[`@Org.OData.Capabilities.V1.FilterRestrictions`].FilterExpressionRestrictions.some(
							(FilterExpressionRestriction: any) => {
...
function prepareEntityType(entityTypeDefinition: any, entityTypeName: string, namespacePrefix: string, metaModelData: any): RawEntityType {
	...
	if (
		...
		!metaModelData.$Annotations[entityType.fullyQualifiedName]?.[`@${UIAnnotationTerms.HeaderInfo}`]
	) {
		...
		metaModelData.$Annotations[entityType.fullyQualifiedName][`@${UIAnnotationTerms.HeaderInfo}`] = headerInfoAnnotation;
	}
...
export function deleteModelCacheData(oMetaModel: ODataMetaModel) {
	if (oMetaModel) {
		delete mMetaModelMap[(oMetaModel as unknown as { id: string }).id];
	}
}
