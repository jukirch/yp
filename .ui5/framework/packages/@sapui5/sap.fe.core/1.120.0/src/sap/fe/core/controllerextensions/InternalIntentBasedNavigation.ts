...
class InternalIntentBasedNavigation extends ControllerExtension {
	...
	@publicExtension()
	@finalExtension()
	navigate(semanticObject: string, action: string, navigationParameters: NavigationParameters | undefined): void {
		const _doNavigate = (oContext?: Context): void => {
	 ...
	 */
	_getAggregates(sMetaPath: string, oView: FEView, oDiagnostics: Diagnostics) {
	 ...
	 * @returns - ConverterContext
	 */
	_getConverterContext(sMetaPath: string, oView: FEView, oDiagnostics: Diagnostics) {
