...
import type CardType from "sap/ui/integration/widgets/Card";
		...
		if (this.aKPIDefinitions && this.aKPIDefinitions.length) {
			...
			this.aKPIDefinitions.forEach((kpiDefinition) => {
				...
				if (kpiDefinition.type === "Analytical") {
					...
					this.loadKPITagData(kpiDefinition, oAppComponent.getModel() as ODataModel, oKPIModel).catch(function (err: string) {
						Log.error(err);
					});
				}
			});
				...
				Core.loadLibrary("sap/ui/integration", { async: true })
					.then(() => {
						sap.ui.require(["sap/ui/integration/widgets/Card", "sap/ui/integration/Host"], (...params: unknown[]) => {
							const Card = params[0] as typeof CardType;
			...
			if (kpiDefinition) {
				...
				Promise.all(aPromises)
					.then((aResults) => {
						...
						const oPopover = aResults[2] as Popover;
