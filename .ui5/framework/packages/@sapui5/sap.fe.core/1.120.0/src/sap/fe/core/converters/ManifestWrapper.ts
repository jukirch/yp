		...
		return (
			viewConfig.paths.find((path: ViewConfiguration) => {
				if ((path as CustomViewTemplateConfiguration)?.template) {
					...
					return (
						(primary as SingleViewPathConfiguration[]).some(
							(primaryPath) => primaryPath.entitySet && primaryPath.entitySet !== manifestEntitySet
						) ||
						(secondary as SingleViewPathConfiguration[]).some(
							(secondaryPath) => secondaryPath.entitySet && secondaryPath.entitySet !== manifestEntitySet
						)
					);
				} else {
