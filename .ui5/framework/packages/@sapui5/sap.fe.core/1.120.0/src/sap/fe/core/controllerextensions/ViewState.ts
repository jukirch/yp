...
const _mControlStateHandlerMap: Record<string, any> = {
	...
	"sap.ui.mdc.FilterBar": {
		...
		apply: async function (filterBar: FilterBar, controlState: ControlState, navParameter: NavigationParameter, skipMerge?: boolean) {
			try {
				if (controlState) {
					const isInitialStateApplicable = this._isInitialStatesApplicable(controlState?.initialState, filterBar, skipMerge);
					...
					if (navigationType === NavType.hybrid && controlState.fullState !== undefined) {
						const filterBarAPI = filterBar.getParent() as FilterBarAPI,
							xAppStateFilters = await filterBarAPI.convertSelectionVariantToStateFilters(
						...
						await (filterBar.getParent() as FilterBarAPI)._clearFilterValuesWithOptions(filterBar, { clearEditFilter: true });
	...
	"sap.ui.mdc.Table": {
		...
		apply: async function (table: Table, controlState: ControlState, navParameters: NavigationParameter, skipMerge?: boolean) {
			try {
				if (controlState) {
					// Extra condition added to apply the diff state logic for mdc control
					const isInitialStateApplicable = this._isInitialStatesApplicable(
						controlState?.initialState,
						table,
						skipMerge,
						navParameters.navigationType !== NavType.hybrid
					);
	...
	"sap.ui.core.mvc.View": {
		...
		apply: function (oView: View, oControlState: ControlState, oNavParameters: NavigationParameter, skipMerge?: boolean) {
	...
	"sap.sac.df.changeHandler.MultiDimModelChangeHandler": {
		retrieve: function (control: MultiDimModelChangeHandler) {
			return control._getMultiDimModel().serialize("INA_REPOSITORY_DELTA");
		},
		apply: function (control: MultiDimModelChangeHandler, controlState: unknown) {
			if (controlState) {
...
class ViewState extends ControllerExtension {
	...
	@publicExtension()
	@finalExtension()
	async retrieveViewState(): Promise<object | undefined> {
	...
	@extensible(OverrideExecution.After)
	// eslint-disable-next-line @typescript-eslint/no-unused-vars
	onBeforeStateApplied(aPromises: Promise<any>, navigationType?: string) {
		// to be overriden
	}
	...
	@privateExtension()
	@finalExtension()
	async applyControlState(oControl: ManagedObject, oControlState: object, oNavParameters?: object, skipMerge?: boolean): Promise<void> {
		const aControlStateHandlers = this.getControlStateHandler(oControl);
		let oPromiseChain = Promise.resolve();
		...
		try {
			...
			adaptControls
				.filter(function (control) {
					return (
						control &&
						(!variantManagement || variantControls.includes(control.getId())) &&
				...
				.forEach((control) => {
					...
					const externalStatePromise = StateUtil.retrieveExternalState(control as MDCControl);
	...
	_isInitialStatesApplicable(initialState: object, control: FilterBar | Table, skipMerge: boolean, isNavHybrid?: boolean): boolean {
		return (
			initialState &&
