...
function _setNonResponsiveTableWithMixFacetsInformation(subSection: DataVisualizationSubSection, sectionLayout: string): void {
	...
	if (subSections.length === 1) {
		...
		switch (subSections[0].type) {
			case "DataVisualization":
				...
				_setNonResponsiveTableVisualizationInformation(
					sections,
					dataVisualizationSubSection,
					subSectionVisualization,
					sectionLayout
				);
				break;
			case "Mixed":
