...
class TemplateComponent extends UIComponent implements IAsyncContentCreation {
	...
	init() {
		...
		super.init();
		const oStateChangeHandler = function (oEvent: Event<{ control: Control }>): void {
			const oControl = oEvent.getParameter("control");
			if (
				oControl.isA<Table>("sap.ui.mdc.Table") ||
				oControl.isA<FilterBar>("sap.ui.mdc.FilterBar") ||
				oControl.isA<Chart>("sap.ui.mdc.Chart")
			) {
				const oMacroAPI = oControl.getParent() as unknown as { fireStateChange: EventHandler<void> };
