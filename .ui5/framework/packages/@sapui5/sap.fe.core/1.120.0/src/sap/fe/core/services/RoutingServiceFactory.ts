...
export class RoutingService extends Service<RoutingServiceSettings> {
	...
	_fnOnRouteMatched!: EventHandler<Route$MatchedEvent>;
	...
	navigateToContext(
		...
		return this.oRouterProxy
			.navToHash(sTargetPath, false, parameters?.noPreservationCache, parameters?.forceFocus, !isStickyMode)
			.then((bNavigated: any) => {
				if (!bNavigated) {
	...
	_checkIfContextSupportsSemanticPath(oContext: Context) {
		...
		const entitySet = MetaModelConverter.getInvolvedDataModelObjects(oMetaModel.getMetaContext(oContext.getPath()))
	 ...
	 */
	_onRouteMatched(oEvent: Route$MatchedEvent): void {
