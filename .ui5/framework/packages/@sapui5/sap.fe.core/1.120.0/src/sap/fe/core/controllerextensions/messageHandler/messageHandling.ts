...
import type { CustomBasedTableColumn } from "../../converters/controls/Common/Table";
...
function showUnboundMessages(
	...
	if (aTransitionMessages.length === 0 && !aCustomMessages && !bHasEtagMessage) {
		...
		return new Promise(function (resolve) {
			...
			if (
				(oMessage.technicalDetails && !aMessageList.includes(oMessage.technicalDetails.originalMessage.message)) ||
...
function getTableColInfo(oTable: Control, sTableTargetColProperty: string) {
	...
	if (!oTableTargetCol) {
		...
		const oCustomColumn = ((oTable as Table).getControlDelegate() as typeof TableDelegate)
			.getColumnsFor(oTable)
			.find(function (oColumn: CustomBasedTableColumn) {
		...
		if (oCustomColumn) {
			...
			const aColumns = ((oTable as Table).getControlDelegate() as typeof TableDelegate).getColumnsFor(oTable);
