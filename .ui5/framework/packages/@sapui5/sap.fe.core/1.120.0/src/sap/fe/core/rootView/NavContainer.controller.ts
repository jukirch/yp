...
class NavContainerController extends BaseController {
	...
	getInstancedViews(): XMLView[] {
		return this._getNavContainer()
			.getPages()
			.map((page: ComponentContainer | Control) => {
				if (page && page.isA<ComponentContainer>("sap.ui.core.ComponentContainer")) {
					return page.getComponentInstance().getRootControl() as XMLView;
				} else {
					return page as XMLView;
				}
			});
	}
