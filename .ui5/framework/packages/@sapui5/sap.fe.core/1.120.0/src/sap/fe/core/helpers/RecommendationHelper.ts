...
export type RecommendationDataInnerObjectType = {
	value: string | number;
...
type PropertyIncompletenessInfoType = {
	recommendations: RecommendationDataInnerObjectType[];
};

export type InternalPropertyAdditionalValue = {
	additionalValues: RecommendationDataInnerObjectType[];
	recommendations?: RecommendationDataInnerObjectType[];
};
...
export const recommendationHelper = {
	 ...
	 */
	sortRecommendationsData(incompletenessInfo: PropertyIncompletenessInfoType) {
		const recommendationDataSorted: RecommendationDataInnerObjectType[] = incompletenessInfo.recommendations;
		recommendationDataSorted.sort((param1: RecommendationDataInnerObjectType, param2: RecommendationDataInnerObjectType) => {
	 ...
	 */
	transformRecommendationsForInternalStorage(data: InCompletenessInfoType | PropertyIncompletenessInfoType) {
	 ...
	 */
	clearRecommendations: (view: View, inputBindingContext?: Context): void => {
		...
		const createMode = (view.getBindingContext("ui") as Context).getProperty("createMode") as boolean;
		if (currentContext?.getPath() !== inputBindingContext?.getPath() || createMode) {
