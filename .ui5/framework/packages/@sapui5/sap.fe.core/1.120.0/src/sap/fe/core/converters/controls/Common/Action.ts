...
export type AnnotationAction = BaseAction & {
	type: ActionType.DataFieldForIntentBasedNavigation | ActionType.DataFieldForAction | ActionType.Copy;
	annotationPath: string;
	customData?: string;
...
export type CustomAction = CustomElement<
	BaseAction & {
		...
		defaultAction?: string | BaseAction; // Indicates whether a default action exists in this context
	}
>;
...
export const removeDuplicateActions = (actions: BaseAction[]): BaseAction[] => {
	...
	actions.forEach((action) => {
		if (action?.menu?.length) {
			const actionMenu = action.menu as BaseAction[];
