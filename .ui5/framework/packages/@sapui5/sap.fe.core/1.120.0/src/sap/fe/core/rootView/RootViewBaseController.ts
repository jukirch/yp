...
class RootViewBaseController extends BaseController {
	...
	protected restoreFocusFromHistory(): void {
		switch (history.state.focusInfo.type) {
			case "Row":
				const table = Core.byId(history.state.focusInfo.tableId) as unknown as Table;
				const pos = table
					.getRowBinding()
					.getCurrentContexts()
				...
				break;
			default:
				Core.byId(history.state.focusInfo.controlId)?.focus();
		}
	...
	async _fetchTitleValue(sPath: string) {
		...
		const sValuePath = oValueExpression.parts ? oValueExpression.parts[0].path : oValueExpression.path,
			...
			oValueBinding = oModel.bindProperty(sValuePath, oBindingViewContext, { $$groupId: "$auto.Heroes" });
		...
		if (sTextExpression) {
			...
			const fnTextFormatter = oTextExpression.formatter,
				oTextBinding = oModel.bindProperty(sTextPath, oBindingViewContext, { $$groupId: "$auto.Heroes" });
