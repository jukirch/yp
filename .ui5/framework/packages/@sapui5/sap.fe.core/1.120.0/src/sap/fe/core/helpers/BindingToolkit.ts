...
export function setUpConstraints(targetMapping: typeof EDM_TYPE_MAPPING, property: Property | ActionParameter) {
	const constraints: {
		scale?: number;
		precision?: number;
		maxLength?: number;
...
export function formatWithTypeInformation<T>(
	...
	if (outExpression.type === "sap.ui.model.odata.type.String" && isProperty(property)) {
		if (!outExpression.constraints?.isDigitSequence || outExpression.constraints?.nullable === false) {
