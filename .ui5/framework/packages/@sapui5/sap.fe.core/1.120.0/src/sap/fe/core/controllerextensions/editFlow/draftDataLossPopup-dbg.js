...
sap.ui.define(["sap/base/Log", "sap/fe/core/controllerextensions/collaboration/ActivitySync", "sap/fe/core/helpers/EditState", "../../controls/DataLossOrDraftDiscard/DraftDataLossDialog.block"], function (Log, ActivitySync, EditState, DraftDataLossDialogBlock) {
  ...
  async function processFclMode(draftAdminData, fnCancelFunction, oController, processFunctionForDrafts, bSkipBindingToView) {
    ...
    if (draftAdminData.CreationDateTime !== draftAdminData.LastChangeDateTime) {
      return dataLossDialog.open(oController).then(selectedKey => draftDataLossPopup.handleDialogSelection(selectedKey, processFunctionForDrafts, fnCancelFunction, oController, bSkipBindingToView));
    } else {
  ...
  async function processDataLossOrDraftDiscardConfirmation(fnProcessFunction, fnCancelFunction, oContext, oController, bSkipBindingToView) {
    ...
    if (oContext && (oContext.isDeleted() || oContext.getObject()) && (!oDraftRoot && navigationType === NavigationType.BackNavigation || !bIsEditable)) {
      fnProcessFunction();
    } else {
      try {
   ...
   */
  async function saveDocument(controller) {
   ...
   */
  async function discardDraft(controller, skipBindingToView) {
    const context = controller.getView().getBindingContext();
    const params = {
   ...
   * @param skipBindingToView The parameter to skip the binding to the view
   */
  function handleDialogSelection(selectedKey, processFunctionForDrafts, fnCancelFunction, controller, skipBindingToView) {
    switch (selectedKey) {
      case DraftDataLossOptions.Save:
        draftDataLossPopup.saveDocument(controller).then(context => processFunctionForDrafts(context)).catch(function (error) {
          if (error === "Cancel") {
      ...
      case DraftDataLossOptions.Discard:
        draftDataLossPopup.discardDraft(controller, skipBindingToView).then(context => processFunctionForDrafts(context)).catch(function (error) {
