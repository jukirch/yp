...
import type { ActionParameter, ActionParameterInfo } from "./_internal";
import { _addMessageForActionParameter, _validateProperties } from "./_internal";
...
function callAction(
	...
	return new Promise(async function (resolve: (value: any) => void, reject: (reason?: any) => void) {
		...
		if (oAction.getObject("$IsBound")) {
			if (mParameters.additionalSideEffect?.pathExpressions?.length > 0) {
		...
		if (fnDialog) {
			...
			try {
				...
				if (
					...
					isChangeSetAndHasErrorsOrWarnings(mParameters.bGrouped)
				) {
...
function actionParameterShowMessageCallback(
	...
	const APDmessages = messages.filter(function (message: any) {
		return (
			message.getTargets &&
			message.getTargets()?.[0].indexOf(mParameters.actionName) !== -1 &&
			mParameters?.aActionParameters?.some(function (actionParam: any) {
				return message.getTargets()?.[0].indexOf(actionParam.$Name) !== -1;
			})
		);
	});
	...
	if (showGenericErrorMessageForChangeSet && !errorTargetsInAPD) {
		...
		if (nonErrorMessageExistsInDialog !== 1 && nonErrorMessageExistsInModel !== -1 && mParameters?.aContexts?.length > 1) {
	...
	if (!hasChangeSetModifiedMessage) {
		if (
			messages.length === 1 &&
			messages[0].getTargets &&
			messages[0].getTargets()?.[0] !== undefined &&
			messages[0].getTargets()?.[0] !== ""
		) {
							...
							try {
								...
								try {
									...
									if (
										...
										try {
											failedContexts = (
												strictHandlingUtilities.strictHandlingTransitionFails as {
													oAction: { getContext: () => Context };
												}[]
											).map((fail) => fail.oAction.getContext());
								...
								await messageHandler.showMessages({
									context: failedContexts[0], // only used in case of failed context to reset in messageHandling showUnboundMessages
									isActionParameterDialogOpen: oDialog.isOpen(),
 ...
 */
function update412TransistionMessages(
	oAction: Control,
	sGroupId: string,
	strictHandlingUtilities: StrictHandlingUtilities,
	mParameters?: StrictHandlingParameters
) {
	const messages: Array<Message> = sap.ui.getCore().getMessageManager().getMessageModel().getData();
	const { processedMessageIds, delaySuccessMessages, strictHandlingTransitionFails } = strictHandlingUtilities;
 ...
 */
function isChangeSetAndHasErrorsOrWarnings(isChangeSet: boolean): boolean {
	let messages: Message[] = [];

	if (isChangeSet) {
		const unBoundMsgs: Message[] = messageHandling.getMessages();
		const boundMsgs: Message[] = messageHandling.getMessages(true);
		messages = [...unBoundMsgs, ...boundMsgs];
	}

	return messages.some(function (message: Message) {
		return message.getType() === "Error" || message.getType() === "Warning";
	});
}
		...
		return new Promise(function (resolve: (value: any) => void) {
			...
			async function fnExecuteSequentially(contextsToExecute: Context[]) {
				...
				function processOneAction(context: any, actionIndex: any, iContextLength: any) {
					...
					const initialTableContext = mParameters.control?.getRowBinding?.()?.getHeaderContext();
					return fnExecuteSingleAction(
						oAction,
						actionIndex,
						{
							context:
								initialTableContext && initialTableContext.getPath() === context.getPath() ? initialTableContext : context,
							pathExpressions: mParameters.additionalSideEffect && mParameters.additionalSideEffect.pathExpressions,
							triggerActions: mParameters.additionalSideEffect && mParameters.additionalSideEffect.triggerActions
						},
						iContextLength
