...
class FclController extends BaseController {
	...
	getInstancedViews(): XMLView[] {
		...
		return componentContainers.map((oPage) => {
			if (oPage && oPage.isA<ComponentContainer>("sap.ui.core.ComponentContainer")) {
				return oPage.getComponentInstance().getRootControl() as XMLView;
			} else {
				return oPage as XMLView;
			}
		});
	}
