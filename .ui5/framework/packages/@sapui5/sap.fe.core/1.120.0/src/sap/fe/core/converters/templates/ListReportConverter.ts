...
import type {
	...
	MultiDimensionalGridViewConfiguration,
...
import type {
	...
	MultiDimensionalGridVisualization
...
import {
	...
	getMultiDimensionalGridVisualization,
...
export type ListReportDefinition = {
	...
	multiDimensionalGridIds: { id?: string; variantManagementId?: string };
	...
	if (config.converterContext) {
		...
		const createVisualization = function (currentPresentation: DataVisualizationDefinition, isPrimary?: boolean) {
			...
			for (const visualization of currentPresentation.visualizations) {
				if (
					isPrimary &&
					(visualization.type === VisualizationType.Chart || visualization.type === VisualizationType.MultiDimensionalGrid)
				) {
			...
			if (defaultVisualization) {
				presentationCreated.visualizations = [defaultVisualization] as
		...
		const getPresentation = function (
			item: SingleViewPathConfiguration | MultiDimensionalGridViewConfiguration,
			isPrimary: boolean
		): DataVisualizationDefinition {
			if ((item as MultiDimensionalGridViewConfiguration).type === "MultiDimensionalGrid") {
				return getMultiDimensionalGridVisualization(item as MultiDimensionalGridViewConfiguration);
			}
			item = item as SingleViewPathConfiguration;
		...
		const createAlpView = function (
			...
			chartControlId =
				primaryVisualization?.visualizations[0]?.type === VisualizationType.Chart
					? primaryVisualization?.visualizations[0]?.id
					: "";
...
export const convertPage = function (converterContext: ConverterContext): ListReportDefinition {
	...
	const targetControlIds = [
		...
		...(multiDimensionalGridIds.variantManagementId ? [multiDimensionalGridIds.variantManagementId] : [])
	];
...
function getContentAreaId(converterContext: ConverterContext, views: ListReportViewDefinition[]): ContentAreaID | undefined {
	...
	if (
		...
		for (const lrView of views) {
			const view: CombinedViewDefinition = lrView as CombinedViewDefinition;
		...
		for (const lrView of views) {
			const view: SingleViewDefinition = lrView as SingleViewDefinition;
 ...
 */
function getMultiDimensionalGridIds(views: ListReportViewDefinition[]): { id?: string; variantManagementId?: string } {
	for (const view of views) {
		const gridVisualization = (view as CombinedViewDefinition)?.primaryVisualization
			?.visualizations?.[0] as MultiDimensionalGridVisualization;
		if (gridVisualization?.type === VisualizationType.MultiDimensionalGrid) {
			return {
