sap.ui.define([
...
], function(deepExtend, FeError, FeLogger) {
	"use strict";
	...
	function getLineItemFromVariant(oMetaModel, sEntityType, sQualifier){
		...
		// Qualifier could be for a SelectionPresentationVariant or for a PresentationVariant. SelectionPresentationVariant has precedence
	...
	*/
	function getTargetEntityForQuickView(oModel, oEntitySet) {
		var oTargetEntityInfo = {
			mTargetEntities: {},
			mProperty: {}
		};
		function formatTargetEntity() {
			return {
				mTargetEntities: oTargetEntityInfo.mTargetEntities,
				sForceLinkRendering: JSON.stringify(oTargetEntityInfo.mProperty) 
			};
		}
			...
			if (oEntityType.name !== oEntitySet.entityType) {
				...
				if (oTargetEntityType["com.sap.vocabularies.UI.v1.QuickViewFacets"]) {
					...
					for (var j = 0; j < aTargetProperties.length; j++) {
						//if property has a semantic object annotation
		...
		aPropertyListWithSemanticObject.forEach(function(sProperty) {
			...
			if (oTargetEntity) {
				oTargetEntityInfo.mTargetEntities[sProperty] = oTargetEntity;
				oTargetEntityInfo.mProperty[sProperty] = true;
			}
		});
