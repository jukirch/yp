sap.ui.define([
	...
	"sap/ui/model/Filter"
], function (BaseObject, MessageUtils, extend, testableHelper, CRUDHelper, isEmptyObject, Filter) {
	"use strict";
	...
	function getMethods(oState, oController, oTemplateUtils) {
		...
		function fnRemoveOldMessageFromModel() {
			var oContextFilter = fnGetFilterForCurrentState(false);
		...
		function fnSavePopUpDialog() {
			...
			oCheckPromise.then(function () {
				...
				if (!bMessageModelContainsError && oTemplateUtils.oComponentUtils.isDraftEnabled()) { //Draft save
					...
					oFilter = fnGetFilterForCurrentState(true);
					...
					oFilter = fnGetFilterForCurrentState(false);
		...
		function fnCreateWithDialog(oCreateWithDialog, oEventSource, oPredefinedValues) {
			oTable = oTemplateUtils.oCommonUtils.getOwnerControl(oEventSource, true);
