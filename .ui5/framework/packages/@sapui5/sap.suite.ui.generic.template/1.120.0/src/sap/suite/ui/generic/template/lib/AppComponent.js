...
sap.ui.define([
	...
	"sap/suite/ui/generic/template/lib/insights/CardProvider",
	...
	PageLeaveHandler,
	ShareUtils,
	CollaborationHelper, CardHelper, CardProvider) {
	"use strict";
	...
	function getMethods(oAppComponent, oAppId) {
		...
		function setGlobalTemplateModels(){
			...
			CollaborationHelper.isTeamsModeActive().then(function(bIsActive) {
				...
				if (!bIsActive) { // If MSTeams mode is active then do not load the insights service
					CardHelper.getServiceAsync("UIService").then(function(oInstance) {
						...
						oTemplateContract.oInsightsCardProvider = new CardProvider(oTemplateContract);
						oTemplateContract.oInsightsCardProvider.fnRegisterApplicationAsCardProvider();
		...
		function init(){
			...
			Promise.all([oExpandHashPromise,oMessageScopeSupportedPromise]).then(function(aResolvedPromiseValue){
				...
				if (!bIsReadOnly && oUshellContainer) {
