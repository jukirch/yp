sap.ui.define([
	...
	function getMethods(oMultipleViewsHandler, oController, oTemplateUtils, oMessageButtonHelper) {
		...
		/**
		 * Sets a technical message to a table.
		 * 
		 * Technical message is a sub-category of custom message and it can be only set by the framework.
		 *
		 * It compares the new technical message with the old message
		 * Case 1: Old one is also a technical message
		 * 	- If the new message has more or equal severity than old one, overrides the old message and keeps the new message in the message strip
		 *  - Otherwise (new message has less severity), skips the process
		 * Case 2: Old one is a custom message
		 * 	- If the new message has more severity than the old one, overrides the old message and keeps the new message in the message strip
		 * 	- Otherwise (new message has less or equal severity), skips the process
		 * 
		 * @param {object} oMessage - Message object
		 * @param {string} oMessage.message - The message text
		 * @param {sap.ui.core.MessageType} oMessage.type - The message severity
		...
		function setTechnicalMessage(oMessage, sSmartTableId) {
			...
			var oOldTechnicalMessage = oMeta.technicalMessage;
			oMeta.technicalMessage = oMessage;
			if (oCurrentCustomMessage && oMessage) {
		...
		function setCustomMessage(oMessage, sSmartTableId, vTabKey, onClose){
			...
			if (oMeta.implementingHelper) {
				...
				if (!oMessage && oMeta.technicalMessage) {
					oMessage = oMeta.technicalMessage;
				} 
		...
		return {
			...
			onClose: onClose,
			setTechnicalMessage: setTechnicalMessage
		};
	}
