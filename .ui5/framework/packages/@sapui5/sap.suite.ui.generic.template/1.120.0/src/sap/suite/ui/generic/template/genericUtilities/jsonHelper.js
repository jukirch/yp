sap.ui.define(["sap/ui/core/date/UI5Date"], function(UI5Date) {
	...
	function fnAccumulateReplaceInfo(vObject, aReplaces, aPath){
		...
		if (vObject instanceof Date){ // case 2
			...
			// -> In order to be able to restore the enumerable properties of vObject they need to be stored within oReplacement, too.
	...
	* - Instances of Date will be recreated using sap.ui.core.date.UI5Date.getInstance(). Hence, they may or may not be instances of the subclass sap.ui.core.date.UI5Date.
	...
	*/
	...
	function getReplacerForDateFunction(oReplaceInfo, oReplaceTarget){
		if (Number.isInteger(oReplaceInfo.milliseconds)){ // Check whether oReplaceInfo is really a replacement info for a Date
			var bUseUI5Date = !oReplaceInfo.childProps || !(oReplaceInfo.childProps.hasOwnProperty("sTimezoneID") || oReplaceInfo.childProps.hasOwnProperty("oDate") || oReplaceInfo.childProps.hasOwnProperty("oDateParts"));
			var dReplace = bUseUI5Date ? UI5Date.getInstance(oReplaceInfo.milliseconds) : new Date(oReplaceInfo.milliseconds);
			if (JSON.stringify(dReplace) !== '"' + oReplaceTarget + '"'){
				return null; // dReplace and the current value do not fit to each other
			}							
			if (oReplaceInfo.childProps && typeof oReplaceInfo.childProps !== "object"){
				return null; // The childProps property is there but does not have the expected structure 
			}
			return function(){
		...
		return null; // oReplaceInfo cannot be a valid replacement for a Date		
	}
	...
	function fnDeStringify(vObject){
				...
				if (i === oReplaceInfo.path.length - 1){ // do not step down the last step in hierarchy, since we want to do a replacement for that property
					switch (oReplaceInfo.type){
						case "undefined":
							...
							return false;
						case "Date":
							var fnDateReplacer = getReplacerForDateFunction(oReplaceInfo, vCurrent[sProp]);
							if (fnDateReplacer){
								fnReplacer = function(){
									vCurrent[sProp] = fnDateReplacer();	
								};
								return false;
							}
							return true;
		...
		if (oRootReplacement){ // there is just one replacement info and that applies to the root itself -> The instance to be created is either undefined or a Date
			if (oRootReplacement.type === "undefined"){
				return undefined;
			} else if (oRootReplacement.type === "Date"){
				var fnReplacer = getReplacerForDateFunction(oRootReplacement, vObject.data);
				return fnReplacer ? fnReplacer() : vObject;
			}
