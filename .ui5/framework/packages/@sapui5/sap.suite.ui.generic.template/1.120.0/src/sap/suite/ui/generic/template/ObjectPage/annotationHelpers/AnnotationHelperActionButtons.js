...
sap.ui.define([
	...
	// This method should be used as templating time function on draft enabled pages only. It returns a function(sProperty). The purpose of this function is as follows:
	// sProperty should be a property in the DraftAdministrativeData. Then the function returns a string that can be used in a (runtime) expression binding.
	// This string will represent the value of the corresponding property in the administrative data of the main object.
	// aAncestorTreeNode should be bound to parameter>/treeNodeAncestors.
	function getDraftAdminDataAccess(aAncestorTreeNodes){
		if (aAncestorTreeNodes.length === 1){
			return function(sProperty){
				return "${DraftAdministrativeData/" + sProperty + "}";
			};
		}
		var oMainTreeNode = aAncestorTreeNodes[1];
		var sMainModelName = oMainTreeNode.specificModelName; 
		return function(sProperty){
			return "${" + sMainModelName + ">DraftAdministrativeData/" + sProperty + "}";
		};
	}
	...
	return {
		...
		getDraftAdminDataAccess: getDraftAdminDataAccess
	};
}, /* bExport= */ true);
