...
sap.ui.define(["sap/ui/base/Object",
	...
	function getMethods(oTemplateContract, oNavigationControllerProxy) {
		...
		function fnInitSession(bIsFirst){
			...
			if (bIsRestoring){
				...
				var oInitialNavigationPromise = oCrossAppNavigatorPromise.then(function(oCrossAppNavigator){
					return oCrossAppNavigator.isInitialNavigationAsync().then(function(bIsInitialNavigation){
		...
		function fnWillNavigationToNodeShowPlaceholder(oTreeNode){
			return oTreeNode.componentId !== aLastActiveComponentForFCLColumn[Math.min(oTreeNode.fCLLevel, 2)] && (oTreeNode.level > 0 || !oTreeNode.componentId) && (oTreeNode.behaviour && oTreeNode.behaviour.getPlaceholderInfo());
		}
		...
		function fnNavigateForNonDraftCreate(sEntitySet, vPredefinedValues, oPredefinedContext, mAppStates){
			...
			if (vPredefinedValues){
				oPredefinedKeyPromise = oCrossAppNavigatorPromise.then(function(oCrossAppNavigator){
					return oCrossAppNavigator.createEmptyAppStateAsync(oTemplateContract.oAppComponent).then(function(oAppState){
		...
		oTemplateContract.oBusyHelper.setBusy((getParsedShellHashFromFLP() || Promise.resolve()).then(function(oResults){
			...
			var oHistoryStateAvailablePromise = oCrossAppNavigatorPromise.then(function(oCrossAppNavigator){
				return oCrossAppNavigator.createEmptyAppStateAsync(oTemplateContract.oAppComponent, true).then(function(oState){
