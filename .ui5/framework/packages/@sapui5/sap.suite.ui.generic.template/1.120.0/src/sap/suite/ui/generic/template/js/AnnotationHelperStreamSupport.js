sap.ui.define([
    ...
    var oAnnotationHelperStreamSupport = {
		...
		getDeleteStreamVisibility: function(oInterface, oMediaType, bIsDraftEnabled, oEntitySet){
			...
			if (bIsDraftEnabled) {
				return oFieldControl && oFieldControl.Path ? '{= ${' + oMediaType + '} !== "" && ${ui>/editable} && ${' + oFieldControl.Path + '} > "1"}' : '{= ${' + oMediaType + '} !== "" && ${ui>/editable}}';
			} else {
				return oFieldControl && oFieldControl.Path ? '{= ${' + oMediaType + '} !== "" && !${ui>/editable} && ${' + oFieldControl.Path + '} > "1"}' : '{= ${' + oMediaType + '} !== "" && !${ui>/editable}}';
			}
		},
		...
		getFileUploaderVisibility: function(oInterface, bIsDraftEnabled, oEntitySet){
			...
			if (bIsDraftEnabled) {
				return oFieldControl && oFieldControl.Path ? '{= ${ui>/editable} && ${' + oFieldControl.Path + '} > "1"}' : '{= ${ui>/editable}}';
			} else {
				if (typeof bRestrictions === "boolean") {
					...
					if (oFieldControl && typeof oFieldControl.Path === "string") {
						return '{= !${ui>/editable} && ${' + oFieldControl.Path + '} > "1" && ${' + bRestrictions + '}}';
					}
