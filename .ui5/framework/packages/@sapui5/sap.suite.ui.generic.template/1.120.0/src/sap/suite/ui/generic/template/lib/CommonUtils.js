sap.ui.define(["sap/ui/base/Object",
	...
	function getMethods(oController, oServices, oComponentUtils) {
		...
		function fnReplaceFiltersWithGetDownloadUrl(sServiceUrl, oFilterParams) {
			var aFilters = sServiceUrl.split("?");
			aFilters[1] = aFilters[1].split("&").map(function (params) {
				if (params.startsWith("$filter")) {
					return oFilterParams.sFilterParams;
				}
				return params;
			}).join("&");
			sServiceUrl = aFilters.join("?");
			return sServiceUrl;
		}
		...
		*/
		...
		function fnUpdateBreakoutEnablementForAction(mBreakOutActionEnabled, aContexts, oModel, sSelectedTabKey, oControl, oBreakoutAction){
			...
			var bEnabled = !bIsAnalyticalListPage || !!mBreakOutActionEnabled[sControlId].enabled;
		...
		*/
		...
		function fnMergeNavigationKeyPropertiesWithValues(aKeys, oResponse) {
			...
			for (var i = 0, ilength = aKeys.length; i < ilength; i++) {
				...
				for (var j = 0, jlength = aKeys[i].aKeys.length; j < jlength; j++) {
					...
					var sEncodedFieldValueFromODataResponse = encodeURIComponent(oResponse[oDatafield.name]);
		...
		function fnCheckToolbarIntentsSupported(oSmartControl) {
			...
			if (aLinksToCheck.length > 0 && oXApplNavigation) {
				var oDeferredLinks = oXApplNavigation.getLinks(aLinksToCheck);
				oDeferredLinks.done(function(aLinks) {
					...
					for (var i = 0; i < iLinksNumber; i++) {
						...
						var sSemObj = aInternalLinks[i].semanticObject;
						var sAction = aInternalLinks[i].action;

						var oSemObjProp = oTemplatePrivateModel.getProperty("/generic/supportedIntents/" + sSemObj);
