...
sap.ui.define([
	...
	Viewport.prototype._renderLoop = function() {
		...
		if (this._shouldRenderFrame && this.getAutoStartRendering()) {
			...
			if (vsm) {
				...
				var is3D = this.isView3D();
				var isPanoramic = this.isViewport3D() && this._isPanoramicActivated();
	...
	Viewport.prototype._prepareForRenderingRecursive = function(node, geometryClusters, scratchBuf, textureDataArray, textureUpdateResults, force, noClusters) {
		...
		if (!noClusters) {
			...
			ThreeUtils.updateNodeClusterRenderingInfo(node, geometryClusters, scratchBuf, textureDataArray, textureUpdateResults);
		}

		// update children
		var children = node.children;
		for (var i = 0, l = children.length; i < l; i++) {
			this._prepareForRenderingRecursive(children[i], geometryClusters, scratchBuf, textureDataArray, textureUpdateResults, force, noClusters);
		}
	};
	...
	Viewport.prototype.render = function() {
		...
		try {
			...
			if (this._shouldProcessHierarchy) {
				...
				this._prepareForRenderingRecursive(nativeScene, geometryClusters, instanceScratchBuffer, dataTextureArray, textureUpdateResults, false, false);
				...
				for (var iRootChild = 0, lenRootChild = nativeScene.children.length; iRootChild < lenRootChild; ++iRootChild) {
					...
					if (rootChild.name === "DefaultLights") {
						this._prepareForRenderingRecursive(rootChild, geometryClusters, instanceScratchBuffer, dataTextureArray, textureUpdateResults, false, false);
					}
				}
			}
			...
			if (shouldUpdateSelectionBoxes && vsm) {
			...
			if (this._camera.getUsingDefaultClipPlanes() || (nativeCamera.isOrthographicCamera && nativeCamera.zoom <= 0)) {
				...
				if (!boundingBox.isEmpty()) {
					...
					if ((maxCameraDistance / maxSceneDimension) > 100) {
