...
sap.ui.define([
	...
	// constructor requires the maximum number of triangles, lines and points to be known beforehand to preallocate typed arrays
	function MeshAnalyzer(maxTriangles, maxLines, maxPoints) {
		...
		this._vertices = new Float64Array((maxTriangleVertices + maxLineVertices + (maxPoints || 0)) * 3);
		this._vertexCount = 0;
	...
	MeshAnalyzer.prototype.intersectSphere = function(position, radius, allowFace, allowEdge, allowVertex, _allowClosedContour, _allowCurvedEdge) {
		...
		if (!this._triangleCount && !this._lineCount && this._vertexCount && allowVertex) {
			// special case for point clouds
			for (let vi = 0, count = this._vertexCount; vi < count; ++vi) {
				v0.set(this.getVertex(vi));
				vertexDistance = Utils.computePointToPointDistance2(v0, position);
				if ((vertexDistance <= radius2) && (!ret.vertexDistance || vertexDistance < ret.vertexDistance)) {
					ret.vertexDistance = vertexDistance;
					ret.vertexId = vi;
				}
