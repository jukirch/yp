...
sap.ui.define([
	...
	function updateProgress(sceneBuilder, phase, count) {
		...
		// We update the progress bar not on every call but only every 0.5 second. This improves the
		// loading time by up to 8 times! The sap.m.ProgressIndicator control used in
		// sap.ui.vk.Viewer is really slow. Also, it does not make any sense to update the progress
		// bar on each call when there can be hundred thousands and millions calls when loading
		// large models.

		var needUpdate = now - progress.lastUpdateTime >= 500 || phase !== progress.phase || count >= progress.totalCount;
