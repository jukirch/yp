...
sap.ui.define([
	...
	MoveToolGizmo.prototype._getSelectionCenter = function(target) {
		...
		if (this._placementMode === GizmoPlacementMode.OnScreen) {
			...
			if (checkOnScreen.some(function(c) { return c !== false; })) {
				target.copy(fromScreenXYZ(edgePos, camera, width, height));
			}

			// keep position at screen center when reach zoom threshold
			if (posOnScreen.z > 1.0) {
				target.copy(getScreenCenter(camera, width, height));
			}
