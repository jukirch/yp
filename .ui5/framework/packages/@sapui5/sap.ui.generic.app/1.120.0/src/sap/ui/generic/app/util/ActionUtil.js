...
sap.ui.define([
	 ...
	 * @protected
	 */
	ActionUtil.prototype.call = function (sFunctionImportPath, sFunctionImportLabel, bIsDraftEnabled, oSkipProperties, bIsStrict, mAdditionalParamData, sFunctionImportButtonActionButtonText) {
		var that = this;
		return new Promise(function (resolve, reject) {
			...
			// this is a sap.ui.core.mvc.Controller instance
			...
			var fnPrepareParamAndInvokeAction = function () {
				...
				mActionParams = that._prepareParameters(aContexts, bIsDraftEnabled, mAdditionalParamData);
				mActionParams = mActionParams || [{}];
				mActionParams.expand = mAdditionalParamData ? mAdditionalParamData.expand : undefined;
				...
				if (oGetDefaultValuesPromise instanceof Promise) {
					...
					// apply values passed via navigation context
					if (mAdditionalParamData.predefinedValues) {
						var aPredefinedValues = Object.keys(mAdditionalParamData.predefinedValues);
						mActionParams.forEach(function(oActionParam) {
							var aParameterNames = oActionParam.additionalParameters.map(function(oParameter) {return oParameter.name;});
							aPredefinedValues.forEach(function(sPredefinedValue) {
								if (aParameterNames.includes(sPredefinedValue)) {
									oActionParam.parameterData[sPredefinedValue] = mAdditionalParamData.predefinedValues[sPredefinedValue];
								}
							});
							oActionParam.isCreationScenario = mAdditionalParamData.isCreationScenario;
						});
					}
					that._initiateCall(mActionParams, bIsDraftEnabled, bIsStrict);
				}
	...
	/**
	 * Checks the action contexts for pending changes
	 *
	 * @param {array} aActionContexts Array of contexts used for action processing
	 * @private
	 */
	 ...
	 * @param {string} sKey - the string containing the current parameter of the selected row
	...
	ActionUtil.prototype._initiateCall = function (mActionParams, bIsDraftEnabled, bIsStrict) {
		...
		// Check if additionalParameters length is 0 or in case of creation case, if all action's parameter(s) are already provided, then initiate the action call
		if (mActionParam != undefined && ((mActionParam.additionalParameters && mActionParam.additionalParameters.length == 0) || (mActionParam.isCreationScenario && Object.values(mActionParam.parameterData).every(function(vParameterValue) { return !!vParameterValue; })) )) {
			...
			// If additionalParameters exists, then create dialog.
			// In this case, it's necessary to synchronise the draft before the getNewActionContext call as it makes a oModel.callFunction call and
			// if the draft is not synchronized, then MERGE and POST, both requests would get triggered after the completion of delayed draft interval.
			var that = this;
			var oApplicationController = this.getApplicationController();
			(bIsDraftEnabled ? oApplicationController.synchronizeDraftAsync() : Promise.resolve()).then(function() {
				var mParameters = {
					urlParameters: {},
					headers: mHeaders,
				...
				var aEntityContext = that.getContexts();
				var fnChangeSet = oApplicationController._getChangeSetFunc(aEntityContext, that.getOperationGrouping());
	
				var aFuncHandles = aEntityContext.map(function(oContext, index) {
					mParameters.changeSetId = fnChangeSet(index);
					return oApplicationController.getNewActionContext(that._sFunctionImportPath, oContext, mParameters);
				});
				var aContexts = aFuncHandles.map(function (oFuncHandle) {
					return oFuncHandle.context;
				});
	
				Promise.all(aContexts).then(function (aActionContexts) {
					...
					if (bIsStrict) {
						...
						var oParameterDialog = new Dialog({
								...
								press: function () {
									...
									oSmartFormCheckPromise.then(function() {
										if (mParameterForm.hasNoClientErrors()) {
											...
											bActionPromiseDone = that._triggerActionPromise(aFuncHandles, aEntityContext, mActionParam, aActionContexts, bIsStrict);
										}
									});
								}
							}),
							endButton: new sap.m.Button({
								...
								press: function () {
									...
									oParameterDialog.close();
									that._cleanUpContext(aActionContexts);
									that._oActionPromiseCallback.reject();
						...
						if (that.getController().getView().getModel("@i18n")) {
							oParameterDialog.setModel(that.getController().getView().getModel("@i18n"), "@i18n");
						}
						...
						if (aFormElements.length === 0 || bIsEmptyPopup){
							that._triggerActionPromise(aFuncHandles, aEntityContext, mActionParam, aActionContexts, bIsStrict);
							oParameterDialog.destroy();
						} else {
							oParameterDialog.open();
						}
					} else {
						that._triggerActionPromise(aFuncHandles, aEntityContext, mActionParam, aActionContexts);
					}
				});
			});
		} else {
	...
	ActionUtil.prototype._triggerActionPromise = function (oFuncHandles, aEntityContext, mActionParam, aActionContexts, bIsStrict) {
		...
		mActionParam.additionalParameters.forEach(function (param) {
			...
			// Case 1: When the field remains untouched, and the submit button is pressed, the data inside the parameter will be "undefined." When the model attempts to create the payload, this parameter will not be included in the payload.
			// Case 2: If the user enters a value and then deletes it, the data inside the parameter will be "null." In this case, when the payload is generated, the model will not remove the parameter, as it did in the previous case. Instead, it will pass the parameter as "null" in the payload.
			// It's important to note that currently, the gateway does not accept the parameter as "null" in the function import scenario.
			// Following a discussion with the Gateway and OData V2 colleagues, it has been agreed that removing the parameter would indicate to the backend that the data should be deleted from the database. Additionally, this change will prevent the Gateway from throwing an error. This can be achieved by setting the parameter to "undefined" whenever a null value is encountered. In this context, a null value in the parameter signifies that the value has been removed by backspacing.
			var _dataObjectValue = _dataObject[param.name] === null ? undefined : _dataObject[param.name];
