...
sap.ui.define([
	...
	"sap/ui/core/Core",
	"sap/ui/core/Lib"
], function (jQuery, library, MobileLibrary,ImageHelper, Control, ProcessFlowConnection, ProcessFlowLaneHeader, Device, ResizeHandler, Log, KeyCodes, ProcessFlowRenderer, UI5Date, Core, CoreLib) {
	"use strict";
	...
	ProcessFlow.prototype.onAfterRendering = function () {
		...
		if ($content && $content.length) {
			...
			if (this._headerHasFocus) {
				...
				if ($nodeToFocus[0]) {
					var oNodeToFocus = Core.byId($nodeToFocus[0].id);
	...
	ProcessFlow.prototype.onfocusin = function (oEvent) {
		...
		if (oEvent && oEvent.target && oEvent.target.id) {
			oTarget = Core.byId(oEvent.target.id);
		}
