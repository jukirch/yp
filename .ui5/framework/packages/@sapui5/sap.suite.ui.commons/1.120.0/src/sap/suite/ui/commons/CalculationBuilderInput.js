sap.ui.define([
	...
	"sap/ui/core/Lib"
			 ...
			 CustomData, Sorter, JSONModel, Device, containsOrEquals, MobileLibrary, Core, CoreLib) {
	"use strict";
					...
					if (that._sSuggestionText.length > 0) {
						...
						if (that._oSuggestionList.getItems().length === 0) {
							...
							if (!that._oPopup.isOpen()) {
								var bIsRtl = Core.getConfiguration().getRTL(),
	...
	CalculationBuilderInput.prototype.onsapfocusleave = function (oEvent) {
		if (oEvent.relatedControlId && containsOrEquals(this._oPopup.getDomRef(), Core.byId(oEvent.relatedControlId).getFocusDomRef())) {
	...
	CalculationBuilderInput.prototype._suggestionListKeyHandling = function (mParameters) {
		...
		Core.byId(aItems[this._iSuggestionSelectedIndex].id).setSelected(true).focus();
