...
sap.ui.define([
	...
	"sap/ui/core/Lib"
			 ...
			 Parameters, Log, Measurement, uid, encodeXML, GraphRenderer, ToggleButton, HashChanger, MobileLibrary, Dialog, FullScreenUtil,InvisibleMessage,coreLibrary, Core, CoreLib) {
	"use strict";
	...
	Graph.prototype.init = function () {
		...
		this._bIsRtl = Core.getConfiguration().getRTL();
	...
	Graph.prototype.onAfterRendering = function () {
		...
		if (this._isDelayedLayouting()) {
			var fnProcess = function () {
				...
				Core.detachThemeChanged(fnProcess);
			}.bind(this);

			// wait for theme to load
			if (!Core.isThemeApplied()) {
				Core.attachThemeChanged(fnProcess);
			} else {
				fnProcess();
			}
		}
	...
	Graph.prototype._createLegend = function () {
		...
		if (Core.byId(this.getId() + "-legendButton").getPressed()) {
